...
export const actions = {
  load: (getInitialData, createAndFireAnalyticsEvent) => ({
    ...
    const promise = getInitialData(`/rest/api/latest/jql/autocompletedata`).then(res => {
      const initialValue = {
        jqlSearchableFields: [],
        jqlOrderableFields: [],
    ...
    setState({
      ...
      jqlSearchableFields$: from(promise).pipe(concatMap(({
        jqlSearchableFields
      }) => jqlSearchableFields)),
      jqlOrderableFields$: from(promise).pipe(concatMap(({
        jqlOrderableFields
      }) => jqlOrderableFields)),
      jqlFunctions$: from(promise).pipe(concatMap(({
        jqlFunctions
      }) => jqlFunctions))
    });
  }
...
export const JQLAutocompleteContainer = createContainer(store);
export const useJqlAutocompleteActions = createHook(store, {
  selector: null
});
export const useJqlSearchableFieldsObservable = createHook(store, {
  selector: ({
    jqlSearchableFields$
  }) => jqlSearchableFields$
});
export const useJqlOrderableFieldsObservable = createHook(store, {
  selector: ({
    jqlOrderableFields$
  }) => jqlOrderableFields$
});
export const useJqlFunctionsObservable = createHook(store, {
  selector: ({
    jqlFunctions$
  }) => jqlFunctions$
});