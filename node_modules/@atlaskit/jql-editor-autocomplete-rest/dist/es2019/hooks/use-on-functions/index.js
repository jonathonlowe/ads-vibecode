...
const useOnFunctions = (jqlSearchableFields$, jqlFunctions$) => {
  return useCallback((query, field, isListOperator) => {
    ...
    return field$.pipe(concatMap(matchingField => jqlFunctions$.pipe(filter(func => !filterRegex || func.displayName.match(filterRegex) !== null), filter(func =>
    // Do not include functions that return a list unless we are using a list operator
    !!isListOperator || func.isList !== 'true'), filter(func =>
    // Include functions with a type that intersects our field types
    func.types.filter(type => matchingField.types.includes(type)).length > 0), map(func => ({
