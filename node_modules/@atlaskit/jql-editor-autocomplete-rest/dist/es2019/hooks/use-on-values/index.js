...
const useOnValues = (jqlSearchableFields$, getSuggestions, createAndFireAnalyticsEvent) => {
  ...
  return useCallback((query, field) => {
    ...
    return field$.pipe(filter(matchingField => matchingField.auto === 'true'), concatMap(matchingField => {
      ...
      // return no data.
      const fieldNameForFetch = (_matchingField$cfid = matchingField.cfid) !== null && _matchingField$cfid !== void 0 ? _matchingField$cfid : normalizedField;
      ...
      return of(null).pipe(delay(OPERANDS_DELAY_MS), concatMap(() => fetchFieldValues(fieldNameForFetch, normalizedQuery).then(values => {
