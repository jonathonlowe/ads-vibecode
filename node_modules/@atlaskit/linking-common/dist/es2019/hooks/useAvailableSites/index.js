  ...
  useEffect(() => {
    const fetchSites = async () => {
      try {
        ...
        createAnalyticsEvent(createEventPayload('operational.getAvailableSitesResolve.failed', getOperationFailedAttributes(err))).fire(ANALYTICS_CHANNEL);
...
async function getAvailableSites({
  ...
  const response = await window.fetch(gatewayBaseUrl ? `${gatewayBaseUrl}/gateway/api/available-sites` : '/gateway/api/available-sites', requestConfig);
  if (response.ok) {
