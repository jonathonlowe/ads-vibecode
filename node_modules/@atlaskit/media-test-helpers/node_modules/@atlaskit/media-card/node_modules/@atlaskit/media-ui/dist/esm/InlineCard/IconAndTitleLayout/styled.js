import { __makeTemplateObject } from "tslib";
import styled from 'styled-components';
// TODO: remove this override behaviour for @atlaskit/icon-object
export var IconObjectOverrides = "\n  & > span {\n    height: 16px;\n    width: 14px;\n    position: absolute;\n    top: 0;\n    left: 0;\n    line-height: 14px;\n    & > svg {\n      position: absolute;\n      top: 50%;\n      left: 50%;\n      transform: translate(-50%, -50%);\n    }\n  }\n";
// TODO: remove this override behaviour for @atlaskit/icon
export var IconOverrides = "\n  & > span > span {\n    height: 16px;\n    width: 14px;\n    position: absolute;\n    top: 0;\n    left: 0;\n    & > svg {\n      position: absolute;\n      top: 50%;\n      left: 50%;\n      transform: translate(-50%, -50%);\n    }\n  }\n";
// Wraps all icons represented in Inline Links. Icons have three sources/types:
// - JSON-LD: from the generator.icon property coming back from ORS.
// - @atlaskit/icon: for lock icons, unauthorized, etc.
// - @atlaskit/icon-object: for object icons, e.g. repository, branch, etc.
// NB: the first set of overrides style icons imported from @atlaskit/icon-object correctly.
// NB: the second set of overrides style icons imported from @atlaskit/icon correctly.
export var IconWrapper = styled.span(templateObject_1 || (templateObject_1 = __makeTemplateObject(["\n  user-select: none;\n  ", "\n  ", "\n"], ["\n  user-select: none;\n  ", "\n  ", "\n"])), IconOverrides, IconObjectOverrides);
// The main 'wrapping' element, title of the content.
// NB: `white-space` adds little whitespace before wrapping.
// NB: `hyphens` enables hyphenation on word break.
export var IconTitleWrapper = styled.span(templateObject_2 || (templateObject_2 = __makeTemplateObject(["\n  hyphens: auto;\n  white-space: pre-wrap;\n  overflow-wrap: break-word;\n  word-break: break-word;\n"], ["\n  hyphens: auto;\n  white-space: pre-wrap;\n  overflow-wrap: break-word;\n  word-break: break-word;\n"])));
// TODO: Replace overrides with proper AtlasKit solution.
export var LozengeWrapper = styled.span(templateObject_3 || (templateObject_3 = __makeTemplateObject(["\n  display: inline-block;\n  vertical-align: 1px;\n  & > span {\n    margin-left: 4px;\n    padding: 2px 0 1px 0;\n  }\n"], ["\n  display: inline-block;\n  vertical-align: 1px;\n  & > span {\n    margin-left: 4px;\n    padding: 2px 0 1px 0;\n  }\n"])));
// TODO: Replace overrides with proper AtlasKit solution.
export var LozengeBlockWrapper = styled.span(templateObject_4 || (templateObject_4 = __makeTemplateObject(["\n  & > span {\n    margin-left: 4px;\n    padding: 2px 0 2px 0;\n  }\n"], ["\n  & > span {\n    margin-left: 4px;\n    padding: 2px 0 2px 0;\n  }\n"])));
// The following components are used to absolutely position icons in the vertical center.
// - IconPositionWrapper: the `relative` parent which has no height in itself.
// - IconEmptyWrapper: the child which forces `IconPositionWrapper` to have a height.
export var IconPositionWrapper = styled.span(templateObject_5 || (templateObject_5 = __makeTemplateObject(["\n  margin-right: 4px;\n  position: relative;\n  display: inline-block;\n"], ["\n  margin-right: 4px;\n  position: relative;\n  display: inline-block;\n"])));
export var IconEmptyWrapper = styled.span(templateObject_6 || (templateObject_6 = __makeTemplateObject(["\n  width: 14px;\n  height: 100%;\n  display: inline-block;\n  opacity: 0;\n"], ["\n  width: 14px;\n  height: 100%;\n  display: inline-block;\n  opacity: 0;\n"])));
var templateObject_1, templateObject_2, templateObject_3, templateObject_4, templateObject_5, templateObject_6;
//# sourceMappingURL=styled.js.map