"use strict";
Object.defineProperty(exports, "__esModule", { value: true });
var tslib_1 = require("tslib");
/** @jsx jsx */
var core_1 = require("@emotion/core");
var react_intl_1 = require("react-intl");
var colors_1 = require("@atlaskit/theme/colors");
var Frame_1 = require("../components/Frame");
var Thumbnail_1 = require("../components/Thumbnail");
var Provider_1 = require("../components/Provider");
var Name_1 = require("../components/Name");
var Byline_1 = require("../components/Byline");
var Content_1 = require("../components/Content");
var constants_1 = require("../utils/constants");
var messages_1 = require("../../messages");
var ContentFooter_1 = require("../components/ContentFooter");
var Icon_1 = require("../components/Icon");
var ContentHeader_1 = require("../components/ContentHeader");
var handlers_1 = require("../utils/handlers");
var textTitleProps = tslib_1.__assign({}, messages_1.messages.not_found_title);
var textDescriptionProps = tslib_1.__assign({}, messages_1.messages.not_found_description);
exports.NotFoundView = function (_a) {
    var _b = _a.context, context = _b === void 0 ? { text: '' } : _b, _c = _a.isSelected, isSelected = _c === void 0 ? false : _c, testId = _a.testId, icon = _a.icon, _d = _a.link, link = _d === void 0 ? '' : _d, _e = _a.onClick, onClick = _e === void 0 ? function () { } : _e;
    var handleClick = function (event) { return handlers_1.handleClickCommon(event, onClick); };
    return (core_1.jsx(Frame_1.Frame, { isSelected: isSelected, testId: testId },
        core_1.jsx(Content_1.Content, null,
            core_1.jsx("div", null,
                core_1.jsx(ContentHeader_1.ContentHeader, { onClick: handleClick, link: link },
                    core_1.jsx(Icon_1.Icon, tslib_1.__assign({}, icon)),
                    core_1.jsx(Name_1.Name, { name: core_1.jsx(react_intl_1.FormattedMessage, tslib_1.__assign({}, textTitleProps)), testId: testId ? testId + "-name" : undefined })),
                core_1.jsx(Byline_1.Byline, { text: core_1.jsx(react_intl_1.FormattedMessage, tslib_1.__assign({}, textDescriptionProps)) })),
            core_1.jsx(ContentFooter_1.ContentFooter, null,
                core_1.jsx(Provider_1.Provider, { name: context.text, icon: context.icon }))),
        core_1.jsx(Thumbnail_1.Thumbnail, { testId: testId ? testId + "-thumb" : undefined, src: constants_1.NotFoundImage, color: colors_1.R50 })));
};
//# sourceMappingURL=NotFoundView.js.map