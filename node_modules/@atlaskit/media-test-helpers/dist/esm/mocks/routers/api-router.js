...
var RouterWithLogging = /** @class */ (function (_super) {
    __extends(RouterWithLogging, _super);
    ...
    RouterWithLogging.prototype.register = function (method, path, originalHandler) {
        var handler = function (request, database) {
            ...
            try {
                ...
                requestWithBodyObject = { request: __assign(__assign({}, request), { body: body }) };
            }
            catch (e) {
                error = e;
            }
...
export function createApiRouter(isSlowServer) {
    var requestDelay = isSlowServer ? 500 : 10;
    ...
    router.post('/file/binary', function (_a, database) {
        ...
        return {
            data: __assign({ id: item.id }, item.details),
        };
    });
    ...
    router.post('/chunk/probe', function (_a, database) {
        ...
        return new KakapoResponse(200, {
            data: {
                results: __spread(existingChunks.map(function () { return ({ exists: true }); }), nonExistingChunks.map(function () { return ({ exists: false }); })),
            },
        }, {});
    });
    ...
    router.put('/upload/:uploadId/chunks', function (_a, database) {
        ...
        if (record) {
            database.update('upload', record.id, {
                chunks: __spread(record.data.chunks, chunks),
            });
            return new KakapoResponse(200, undefined, {});
        }
        else {
    ...
    router.post('/file/upload', function (_a, database) {
        ...
        return {
            data: __assign(__assign({}, record.data.details), { id: record.data.id }),
        };
    });
    router.get('/file/:fileId', function (_a, database) {
        ...
        if (record) {
            return {
                data: __assign({ id: fileId }, record.data.details),
            };
        }
        else {
            return new KakapoResponse(404, undefined, {});
