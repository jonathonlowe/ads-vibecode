...
const VolumeRange = props => {
  ...
  const [wrapperElementWidth, _setWrapperElementWidth] = useState(0);
  const wrapperElementWidthRef = React.useRef(wrapperElementWidth);
  const setWrapperElementWidth = width => {
  ...
  const onMouseMove = e => {
    ...
    let absolutePosition = clientX - mouseEventsSharedData.current.dragStartClientX;
    ...
    const newVolumeWithBoundaries = absolutePosition / wrapperElementWidthRef.current;
