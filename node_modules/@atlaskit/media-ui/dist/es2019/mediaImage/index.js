...
export const MediaImage = /*#__PURE__*/forwardRef(({
  dataURI,
  alt = '',
  crop = false,
  stretch = false,
  ...
  const [{
    width: imgWidth,
    height: imgHeight
  }, setImageDimensions] = useState({
    width: 0,
    height: 0
  });
  const [isImageLoaded, setIsImageLoaded] = useState(false);
  const imgRef = useRef(null);
  const imageRef = useCallback(imgElem => {
    ...
    if (imgElem.parentElement) {
      const {
        width,
        height
      } = imgElem.parentElement.getBoundingClientRect();
      setParentDimensions({
        width,
        height
      });
  ...
  const imageLoadHandler = e => {
    ...
    onImageLoad === null || onImageLoad === void 0 ? void 0 : onImageLoad(e.currentTarget);
  };
