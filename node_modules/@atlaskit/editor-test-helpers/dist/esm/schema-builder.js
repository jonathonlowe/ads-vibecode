...
export function text(value, schema) {
    ...
    try {
        for (var _b = __values(matches(value, /([\\]+)?{(\w+|<|>|<>|<cell|cell>|<node>|<\|gap>|<gap\|>)}/g)), _c = _b.next(); !_c.done; _c = _b.next()) {
            var match = _c.value;
            var _d = __read(match, 3), refToken = _d[0], skipChars = _d[1], refName = _d[2];
    ...
    try {
        for (var content_1 = __values(content), content_1_1 = content_1.next(); !content_1_1.done; content_1_1 = content_1.next()) {
            ...
            if (isRefsTracker(node)) {
                refs = __assign(__assign({}, refs), offsetRefs(node.refs, position));
            }
            if (isRefsNode(node)) {
                var thickness = node.isText ? 0 : 1;
                refs = __assign(__assign({}, refs), offsetRefs(node.refs, position + thickness));
...
export function flatten(deep) {
    ...
    var flat = [];
    try {
        for (var deep_1 = __values(deep), deep_1_1 = deep_1.next(); !deep_1_1.done; deep_1_1 = deep_1.next()) {
            var item = deep_1_1.value;
            if (Array.isArray(item)) {
                flat.splice.apply(flat, __spread([flat.length, 0], item));
            }
            else {
                flat.push(item);
            }
...
export function coerce(content, schema) {
    ...
    return sequence.apply(void 0, __spread(flatten(refsContent)));
}
/**
...
export var cleanOne = function (content) { return function (schema) {
