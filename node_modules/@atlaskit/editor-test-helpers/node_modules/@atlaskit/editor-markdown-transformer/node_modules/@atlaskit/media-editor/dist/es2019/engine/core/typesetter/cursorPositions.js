...
export const getCursorPositions = (text, direction, textHelperDiv) => {
    if (text.length === 0) {
        return [0];
    }
...
const detectCursorPositions = (text, direction, rootSpan) => {
...
const initCursorsArray = (length, value = 0) => Array.from({ length }).map(() => value);
// Extracting groups from the text
const getGroups = (text, baseDirection, rootSpan) => {
...
const extractBaseGroups = (text, direction, rootSpan) => {
...
export const getGroupStartIndices = (baseGroups) => {
    let index = 0;
...
export const getGroupXMinMax = (baseGroups, direction, rootSpan) => {
...
export const createSpansForCharacters = (text, parent) => {
...
// Wrap each group into a span
export const createSpansForGroups = (baseGroups, parent) => {
    return baseGroups.map(group => {
        const span = document.createElement('span');
...
export const calculateTotalLength = (groups) => {
    return groups
        .map(group => group.xmax - group.xmin)
        .reduce((prev, curr) => prev + curr, 0);
};
// Setting cursor positions inside a group
const setInternalCursorPositions = (group, rootSpan, cursors) => {
    ...
    if (group.direction === 'rtl') {
        putInternalPositions(group.text, span, cursors, group.startIndex, spanWidth => group.xmax - spanWidth, // for 'rtl' groups we calculate from the right side
        // for 'rtl' groups we calculate from the right side
        pos => Math.max(pos, group.xmin));
    }
    else {
        putInternalPositions(group.text, span, cursors, group.startIndex, spanWidth => group.xmin + spanWidth, // for 'ltr' groups we calculate from the left side
        // for 'ltr' groups we calculate from the left side
        pos => Math.min(pos, group.xmax));
    }
...
export const putInternalPositions = (text, span, cursors, startIndex, posCalculator, posLimiter) => {
