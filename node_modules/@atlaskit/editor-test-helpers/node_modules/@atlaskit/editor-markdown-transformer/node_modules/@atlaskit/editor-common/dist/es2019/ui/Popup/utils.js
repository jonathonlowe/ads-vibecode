...
const calculateHorizontalPlacement = ({ placement, targetLeft, targetRight, targetWidth, isPopupParentBody, popupOffsetParentLeft, popupOffsetParentRight, popupOffsetParentScrollLeft, popupOffsetParentClientWidth, popupClientWidth, offset, allowOutOfBounds = false, }) => {
    const position = {};
    if (placement === 'left') {
        ...
        const left = Math.ceil(targetRight -
            popupOffsetParentLeft +
            (isPopupParentBody ? 0 : popupOffsetParentScrollLeft) +
            offset[0]);
        position.left = left;
...
const calculateVerticalStickBottom = ({ target, targetTop, targetHeight, popup, offset, position, }) => {
...
const calculateVerticalStickTop = ({ target, targetTop, targetHeight, popupOffsetParentHeight, popupOffsetParent, offset, position, placement, }) => {
    const scrollParent = findOverflowScrollParent(target);
    const newPos = { ...position };
    if (scrollParent) {
        ...
        if (topBoundary < 0) {
            const isBelowNodeBoundary = targetTop +
...
const calculateVerticalPlacement = ({ placement, targetTop, targetHeight, isPopupParentBody, popupOffsetParentHeight, popupOffsetParentTop, popupOffsetParentScrollTop, borderBottomWidth, offset, }) => {
    const position = {};
    if (placement === 'top') {
 ...
 */
export function calculatePosition({ placement, target, popup, offset, stick, allowOutOfBounds = false, rect, }) {
    let position = {};
    ...
    const { top: popupOffsetParentTop, left: popupOffsetParentLeft, right: popupOffsetParentRight, height: popupOffsetParentHeight, } = rect ? rect : popupOffsetParent.getBoundingClientRect();
    const { top: targetTop, left: targetLeft, right: targetRight, height: targetHeight, width: targetWidth, } = target.getBoundingClientRect();
