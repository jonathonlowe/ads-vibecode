...
export var getMarksByOrder = function (marks) {
    return __spread(marks).sort(function (a, b) { return markOrder.indexOf(a.type.name) - markOrder.indexOf(b.type.name); });
};
/*
...
var flattenUnknownBlockTree = function (node, schema, adfStage) {
    ...
    for (var i = 0; i < node.content.length; i++) {
        ...
        if (isLeafNode) {
            ...
            output.push.apply(output, __spread(flattenUnknownBlockTree(childNode, schema, adfStage)));
        }
    ...
    if (type) {
        switch (type) {
            case 'applicationCard': {
                ...
                var text_1 = attrs.text, link = attrs.link, background = attrs.background, preview = attrs.preview, title = attrs.title, description = attrs.description, details = attrs.details, actions = attrs.actions, context_1 = attrs.context;
                ...
                if (context_1 && (context_1.icon && !isValidIcon(context_1.icon))) {
                    break;
                }
