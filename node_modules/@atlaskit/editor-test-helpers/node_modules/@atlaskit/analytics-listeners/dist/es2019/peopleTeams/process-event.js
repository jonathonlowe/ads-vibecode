...
export default (event, logger) => {
    ...
    const { packageName, packageVersion } = last(getPackageInfo(event, PEOPLE_TEAMS_CONTEXT)) || {};
    const packageHierarchy = packages.map(p => p.packageVersion ? `${p.packageName}@${p.packageVersion}` : p.packageName);
    const { eventType = UI_EVENT_TYPE, action, actionSubject, actionSubjectId, attributes: payloadAttributes, name, } = event.payload;
    const attributes = {
        listenerVersion,
