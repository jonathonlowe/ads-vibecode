...
function extractPropsFromTextDocument(json) {
    ...
    if (json.fileFormat === 'application/vnd.google-apps.document') {
        props.icon = (react_1.default.createElement(_24_2.default, { label: json.provider ? json.provider.name : 'Google Doc' }));
    }
    else if (json.fileFormat === 'application/vnd.ms-word') {
        props.icon = (react_1.default.createElement(_24_3.default, { label: json.provider ? json.provider.name : 'MS Word' }));
    }
    else {
        props.icon = (react_1.default.createElement(_24_1.default, { label: json.provider ? json.provider.name : 'Text document' }));
    }
    return props;
}
