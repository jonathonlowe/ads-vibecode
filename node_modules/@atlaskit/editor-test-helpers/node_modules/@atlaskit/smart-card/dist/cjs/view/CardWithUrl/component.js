...
function LazyCardWithUrlContent(props) {
    var appearance = props.appearance, isSelected = props.isSelected, container = props.container, url = props.url, testId = props.testId;
    var offset = Math.ceil(window.innerHeight / 4);
    return (react_1.default.createElement(react_lazily_render_1.default, { offset: offset, component: appearance === 'inline' ? 'span' : 'div', placeholder: react_1.default.createElement(media_ui_1.CardLinkView, { isSelected: isSelected, key: 'lazy-render-placeholder', link: url, testId: testId }), scrollContainer: container, content: react_1.default.createElement(CardWithUrlContent, tslib_1.__assign({}, props)) }));
}
exports.LazyCardWithUrlContent = LazyCardWithUrlContent;
function CardWithUrlContent(_a) {
    var url = _a.url, isSelected = _a.isSelected, onClick = _a.onClick, appearance = _a.appearance, dispatchAnalytics = _a.dispatchAnalytics, onResolve = _a.onResolve, testId = _a.testId;
    ...
    return appearance === 'inline' ? (react_1.default.createElement(InlineCard_1.InlineCard, { url: url, cardState: state, handleAuthorize: (services.length && handleAuthorize) || undefined, handleFrameClick: handleClickWrapper, isSelected: isSelected, onResolve: onResolve, testId: testId })) : (react_1.default.createElement(BlockCard_1.BlockCard, { url: url, cardState: state, handleAuthorize: (services.length && handleAuthorize) || undefined, handleErrorRetry: actions.reload, handleFrameClick: handleClickWrapper, isSelected: isSelected, onResolve: onResolve }));
}
