...
function useSmartCardActions(url, dispatchAnalytics) {
        ...
        return tslib_1.__awaiter(this, void 0, void 0, function () {
            ...
            return tslib_1.__generator(this, function (_a) {
                ...
                if (showLoadingSpinner && (!hasAuthorized || !hasData)) {
                    setTimeout(function () {
                        if (!isCompleted) {
                            ...
                            dispatchAnalytics(analytics_1.unresolvedEvent(constants_1.ANALYTICS_RESOLVING, definitionId));
                        }
                ...
                if (isReloading || hasExpired || !hasData) {
                    ...
                    dispatchAnalytics(analytics_1.resolvedEvent(definitionId, true));
                    isCompleted = true;
                }
                return [2 /*return*/];
            });
    ...
    function handleResolvedLinkResponse(resourceUrl, response) {
        ...
        if (nextStatus === 'unauthorized' && config.authFlow !== 'oauth2') {
            handleResolvedLinkError(resourceUrl, new errors_1.FetchError('fallback', constants_1.ERROR_MESSAGE_OAUTH));
            return;
        }
        // Handle any other errors
        if (nextStatus === 'fatal') {
            handleResolvedLinkError(resourceUrl, new errors_1.FetchError('fallback', constants_1.ERROR_MESSAGE_FATAL));
            return;
        }
    ...
    function handleResolvedLinkError(resourceUrl, error) {
        ...
        var nextDefinitionId = details && helpers_1.getDefinitionId(details);
        ...
        if (errorKind === 'fatal') {
            ...
            if (status === 'resolved') {
                ...
                dispatchAnalytics(analytics_1.resolvedEvent(nextDefinitionId, true));
            }
            ...
            dispatchAnalytics(analytics_1.unresolvedEvent(constants_1.ANALYTICS_ERROR, nextDefinitionId));
        }
        else {
            // Fallback to blue link with smart link formatting for any other errors
            var errMessage = error && error.message;
            dispatch(helpers_1.cardAction(constants_1.ACTION_ERROR, { url: resourceUrl }, errMessage));
            if (errorKind === 'fallback') {
                dispatchAnalytics(analytics_1.unresolvedEvent(constants_1.ANALYTICS_FALLBACK, nextDefinitionId));
            }
            else {
                dispatchAnalytics(analytics_1.unresolvedEvent(constants_1.ANALYTICS_ERROR, nextDefinitionId));
            }
