...
export var buildTaskUser = function (json) {
    if (json.assignedBy && (json.assignedBy.image || json.assignedBy.name)) {
        return {
            user: __assign(__assign({}, (json.assignedBy.image ? { icon: json.assignedBy.image } : {})), (json.assignedBy.name ? { name: json.assignedBy.name } : {})),
        };
    }
    return {};
};
...
export var buildTaskCommentCount = function (json) {
    if (!isNaN(Number(json.commentCount)) && Number(json.commentCount) > 0) {
        return {
            icon: (React.createElement(ChatIcon, { label: "", key: "comments-count-icon", size: "medium", primaryColor: N600 })),
...
export var buildTaskContext = function (json) {
    ...
    if (genName) {
        var additional = (json.context &&
            ...
            '';
        return {
            context: __assign({ text: genName + additional }, (json.generator.icon ? { icon: json.generator.icon } : {})),
        };
    }
    return {};
};
export function extractBlockViewPropsFromTask(json) {
    ...
    var props = __assign(__assign(__assign(__assign(__assign(__assign(__assign(__assign({}, buildTaskContext(json)), buildTaskTitle(json)), buildTaskDescription(json)), buildTaskLink(json)), buildTaskByline(json)), buildTaskUser(json)), buildTaskUsers(json)), buildTaskDetails(json));
    return props;
}
