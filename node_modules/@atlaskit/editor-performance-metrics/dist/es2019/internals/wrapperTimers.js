...
export const wrapperTimers = ({
  ...
  const timeoutsUnholdMap = new Map();
  const timeoutAllowed = typeof maxTimeoutAllowed === 'number' ? maxTimeoutAllowed : MAX_TIMEOUT_DELAY_ALLOWED_TO_BE_WRAPPED;
  const nestedTimersAllowed = typeof maxNestedTimers === 'number' ? maxNestedTimers : MAX_NESTED_TIMERS_ALLOWED_TO_BE_WRAPPED;
  ...
  const setTimeoutProxy = new Proxy(globalContext.setTimeout, {
    apply(target, thisArg, args) {
      ...
      const isValidDelayTime = typeof delayTime !== 'number' || delayTime === 0 || delayTime > timeoutAllowed;
      ...
      timerId = target.apply(thisArg, [callbackProxy,
