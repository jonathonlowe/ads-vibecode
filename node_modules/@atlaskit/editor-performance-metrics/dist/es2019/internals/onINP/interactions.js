...
export class InteractionManager {
  ...
  estimateP98LongestInteraction() {
    const candidateInteractionIndex = Math.min(this.longestInteractionList.length - 1, Math.floor(this.interactionCountPolyfill.getInteractionCount() / 50));
  ...
  processInteractionEntry(entry) {
    ...
    const minLongestInteraction = this.longestInteractionList[this.longestInteractionList.length - 1];
    ...
    if (existingInteraction || this.longestInteractionList.length < InteractionManager.MAX_INTERACTIONS_TO_CONSIDER || entry.duration > minLongestInteraction.latency) {
      ...
      if (this.longestInteractionList.length > InteractionManager.MAX_INTERACTIONS_TO_CONSIDER) {
        this.longestInteractionList.splice(InteractionManager.MAX_INTERACTIONS_TO_CONSIDER).forEach(i => this.longestInteractionMap.delete(i.id));
      }
    }
  }
}
