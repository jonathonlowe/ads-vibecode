/**
 * NOTE:
 *
 * This file is automatically generated by Traduki 2.0.
 * DO NOT CHANGE IT BY HAND or your changes will be lost.
 */
declare const _default: {
    'editor-common-extensibility-extension-lozenge-editToggle.done.editing': string;
    'editor-common-extensibility-extension-lozenge-editToggle.make.edits': string;
    'editor-common-extensibility.macro.button.configure': string;
    'fablric.editor.viewAllInserts': string;
    'fabric.editor.action': string;
    'fabric.editor.action.description': string;
    'fabric.editor.addAltText': string;
    'fabric.editor.addColumnLeft': string;
    'fabric.editor.addColumnRight': string;
    'fabric.editor.addCommentOnMedia': string;
    'fabric.editor.addImageBorder': string;
    'fabric.editor.addLoomVideo': string;
    'fabric.editor.addLoomVideoComment': string;
    'fabric.editor.addMediaFiles': string;
    'fabric.editor.addRowAbove': string;
    'fabric.editor.addRowBelow': string;
    'fabric.editor.addUpdatedAltText': string;
    'fabric.editor.ai.ai-panels.actionItemsPageDescription': string;
    'fabric.editor.ai.ai-panels.actionItemsPageTitle': string;
    'fabric.editor.ai.ai-panels.summarizePageDescription': string;
    'fabric.editor.ai.ai-panels.summarizePageTitle': string;
    'fabric.editor.ai.ai-smart-button.context.reference': string;
    'fabric.editor.ai.atlas-prebuilt.atlasGenerateContentConfigItemDescription': string;
    'fabric.editor.ai.atlas-prebuilt.atlasSummarizePageConfigItemDescription': string;
    'fabric.editor.ai.commandPalette.promptBuilderForm.buttons.submit': string;
    'fabric.editor.ai.commandPalette.promptBuilderForm.generateContent.ariaLabel': string;
    'fabric.editor.ai.config.item.action.insert': string;
    'fabric.editor.ai.config.item.action.insertAtTop': string;
    'fabric.editor.ai.config.item.action.insertBelow': string;
    'fabric.editor.ai.config.item.action.replace': string;
    'fabric.editor.ai.config.item.action.replaceTitle': string;
    'fabric.editor.ai.config.item.action.replaceTitleTooLongDisabledTooltip': string;
    'fabric.editor.ai.config.item.action.rovoAgents.continueInChatLabel': string;
    'fabric.editor.ai.config.item.agent.title': string;
    'fabric.editor.ai.config.item.atlas-shorten-update.description': string;
    'fabric.editor.ai.config.item.atlas-shorten-update.title': string;
    'fabric.editor.ai.config.item.bb.pr.description.description': string;
    'fabric.editor.ai.config.item.bb.pr.description.title': string;
    'fabric.editor.ai.config.item.brainstorm.description': string;
    'fabric.editor.ai.config.item.brainstorm.promptHint': string;
    'fabric.editor.ai.config.item.brainstorm.promptLabel': string;
    'fabric.editor.ai.config.item.brainstorm.title': string;
    'fabric.editor.ai.config.item.changeTone.casualTone.description': string;
    'fabric.editor.ai.config.item.changeTone.casualTone.selectionToolbar.dropdownItem.title': string;
    'fabric.editor.ai.config.item.changeTone.casualTone.shortTitle': string;
    'fabric.editor.ai.config.item.changeTone.casualTone.title': string;
    'fabric.editor.ai.config.item.changeTone.educationalTone.description': string;
    'fabric.editor.ai.config.item.changeTone.educationalTone.selectionToolbar.dropdownItem.title': string;
    'fabric.editor.ai.config.item.changeTone.educationalTone.shortTitle': string;
    'fabric.editor.ai.config.item.changeTone.educationalTone.title': string;
    'fabric.editor.ai.config.item.changeTone.empatheticTone.description': string;
    'fabric.editor.ai.config.item.changeTone.empatheticTone.selectionToolbar.dropdownItem.title': string;
    'fabric.editor.ai.config.item.changeTone.empatheticTone.shortTitle': string;
    'fabric.editor.ai.config.item.changeTone.empatheticTone.title': string;
    'fabric.editor.ai.config.item.changeTone.nestingParentTitlePostGA': string;
    'fabric.editor.ai.config.item.changeTone.neutralTone.description': string;
    'fabric.editor.ai.config.item.changeTone.neutralTone.selectionToolbar.dropdownItem.title': string;
    'fabric.editor.ai.config.item.changeTone.neutralTone.shortTitle': string;
    'fabric.editor.ai.config.item.changeTone.neutralTone.title': string;
    'fabric.editor.ai.config.item.changeTone.professionalTone.description': string;
    'fabric.editor.ai.config.item.changeTone.professionalTone.selectionToolbar.dropdownItem.title': string;
    'fabric.editor.ai.config.item.changeTone.professionalTone.shortTitle': string;
    'fabric.editor.ai.config.item.changeTone.professionalTone.title': string;
    'fabric.editor.ai.config.item.convertToBulletList.description': string;
    'fabric.editor.ai.config.item.convertToBulletList.title': string;
    'fabric.editor.ai.config.item.convertToTable.description': string;
    'fabric.editor.ai.config.item.convertToTable.title': string;
    'fabric.editor.ai.config.item.draftReply.description': string;
    'fabric.editor.ai.config.item.draftReply.title': string;
    'fabric.editor.ai.config.item.enhance.fixSpellingGrammar.description': string;
    'fabric.editor.ai.config.item.enhance.fixSpellingGrammar.selectionToolbar.dropdownItem.title': string;
    'fabric.editor.ai.config.item.enhance.fixSpellingGrammar.title': string;
    'fabric.editor.ai.config.item.enhance.improveWriting.description': string;
    'fabric.editor.ai.config.item.enhance.improveWriting.selectionToolbar.dropdownItem.title': string;
    'fabric.editor.ai.config.item.enhance.improveWriting.title': string;
    'fabric.editor.ai.config.item.findActionItems.description': string;
    'fabric.editor.ai.config.item.findActionItems.title': string;
    'fabric.editor.ai.config.item.freeGenerate.disabled.promptHint': string;
    'fabric.editor.ai.config.item.generateContent.promptHint': string;
    'fabric.editor.ai.config.item.improveDescription.default.description': string;
    'fabric.editor.ai.config.item.improveDescription.default.title.experiment.one': string;
    'fabric.editor.ai.config.item.makeLonger.description': string;
    'fabric.editor.ai.config.item.makeLonger.title': string;
    'fabric.editor.ai.config.item.range.agent.description': string;
    'fabric.editor.ai.config.item.range.agent.prompt.hint': string;
    'fabric.editor.ai.config.item.rephrase.description': string;
    'fabric.editor.ai.config.item.rephrase.title': string;
    'fabric.editor.ai.config.item.shorten.description': string;
    'fabric.editor.ai.config.item.shorten.selectionToolbar.dropdownItem.title': string;
    'fabric.editor.ai.config.item.shorten.title': string;
    'fabric.editor.ai.config.item.suggest.description': string;
    'fabric.editor.ai.config.item.suggest.title': string;
    'fabric.editor.ai.config.item.summarize.description': string;
    'fabric.editor.ai.config.item.summarize.dropdownItem.title': string;
    'fabric.editor.ai.config.item.summarize.title': string;
    'fabric.editor.ai.config.item.translate.chineseSimplified': string;
    'fabric.editor.ai.config.item.translate.chineseTraditional': string;
    'fabric.editor.ai.config.item.translate.czech': string;
    'fabric.editor.ai.config.item.translate.danish': string;
    'fabric.editor.ai.config.item.translate.dutch': string;
    'fabric.editor.ai.config.item.translate.english': string;
    'fabric.editor.ai.config.item.translate.finnish': string;
    'fabric.editor.ai.config.item.translate.french': string;
    'fabric.editor.ai.config.item.translate.german': string;
    'fabric.editor.ai.config.item.translate.hungarian': string;
    'fabric.editor.ai.config.item.translate.italian': string;
    'fabric.editor.ai.config.item.translate.japanese': string;
    'fabric.editor.ai.config.item.translate.korean': string;
    'fabric.editor.ai.config.item.translate.nestingParentTitle.ga': string;
    'fabric.editor.ai.config.item.translate.nestingParentTitlePostGA': string;
    'fabric.editor.ai.config.item.translate.norwegian': string;
    'fabric.editor.ai.config.item.translate.polish': string;
    'fabric.editor.ai.config.item.translate.portugueseBrazilian': string;
    'fabric.editor.ai.config.item.translate.russian': string;
    'fabric.editor.ai.config.item.translate.spanish': string;
    'fabric.editor.ai.config.item.translate.swedish': string;
    'fabric.editor.ai.config.item.translate.thai': string;
    'fabric.editor.ai.config.item.translate.turkish': string;
    'fabric.editor.ai.config.item.translate.ukrainian': string;
    'fabric.editor.ai.config.item.translate.vietnamese': string;
    'fabric.editor.ai.config.item.translateTo.chineseSimplified.description': string;
    'fabric.editor.ai.config.item.translateTo.chineseSimplified.title': string;
    'fabric.editor.ai.config.item.translateTo.chineseTraditional.description': string;
    'fabric.editor.ai.config.item.translateTo.chineseTraditional.title': string;
    'fabric.editor.ai.config.item.translateTo.czech.description': string;
    'fabric.editor.ai.config.item.translateTo.czech.title': string;
    'fabric.editor.ai.config.item.translateTo.danish.description': string;
    'fabric.editor.ai.config.item.translateTo.danish.title': string;
    'fabric.editor.ai.config.item.translateTo.dutch.description': string;
    'fabric.editor.ai.config.item.translateTo.dutch.title': string;
    'fabric.editor.ai.config.item.translateTo.english.description': string;
    'fabric.editor.ai.config.item.translateTo.english.title': string;
    'fabric.editor.ai.config.item.translateTo.finnish.description': string;
    'fabric.editor.ai.config.item.translateTo.finnish.title': string;
    'fabric.editor.ai.config.item.translateTo.french.description': string;
    'fabric.editor.ai.config.item.translateTo.french.title': string;
    'fabric.editor.ai.config.item.translateTo.german.description': string;
    'fabric.editor.ai.config.item.translateTo.german.title': string;
    'fabric.editor.ai.config.item.translateTo.hungarian.description': string;
    'fabric.editor.ai.config.item.translateTo.hungarian.title': string;
    'fabric.editor.ai.config.item.translateTo.italian.description': string;
    'fabric.editor.ai.config.item.translateTo.italian.title': string;
    'fabric.editor.ai.config.item.translateTo.japanese.description': string;
    'fabric.editor.ai.config.item.translateTo.japanese.title': string;
    'fabric.editor.ai.config.item.translateTo.korean.description': string;
    'fabric.editor.ai.config.item.translateTo.korean.title': string;
    'fabric.editor.ai.config.item.translateTo.norwegian.description': string;
    'fabric.editor.ai.config.item.translateTo.norwegian.title': string;
    'fabric.editor.ai.config.item.translateTo.polish.description': string;
    'fabric.editor.ai.config.item.translateTo.polish.title': string;
    'fabric.editor.ai.config.item.translateTo.portugueseBrazilian.description': string;
    'fabric.editor.ai.config.item.translateTo.portugueseBrazilian.title': string;
    'fabric.editor.ai.config.item.translateTo.russian.description': string;
    'fabric.editor.ai.config.item.translateTo.russian.title': string;
    'fabric.editor.ai.config.item.translateTo.spanish.description': string;
    'fabric.editor.ai.config.item.translateTo.spanish.title': string;
    'fabric.editor.ai.config.item.translateTo.swedish.description': string;
    'fabric.editor.ai.config.item.translateTo.swedish.title': string;
    'fabric.editor.ai.config.item.translateTo.thai.description': string;
    'fabric.editor.ai.config.item.translateTo.thai.title': string;
    'fabric.editor.ai.config.item.translateTo.turkish.description': string;
    'fabric.editor.ai.config.item.translateTo.turkish.title': string;
    'fabric.editor.ai.config.item.translateTo.ukrainian.description': string;
    'fabric.editor.ai.config.item.translateTo.ukrainian.title': string;
    'fabric.editor.ai.config.item.translateTo.vietnamese.description': string;
    'fabric.editor.ai.config.item.translateTo.vietnamese.title': string;
    'fabric.editor.ai.confluence-prebuilt.confluenceChangeToneSelectionToolbarDropdownMenuTitle': string;
    'fabric.editor.ai.confluence-prebuilt.confluenceRewriteSelectionToolbarDropdownMenuTitle': string;
    'fabric.editor.ai.confluence-prebuilt.confluenceTranslateSelectionToolbarDropdownMenuTitle': string;
    'fabric.editor.ai.eventHub.fallbackSuggestedTitle': string;
    'fabric.editor.ai.eventHub.fallbackSuggestedTitlePrefix': string;
    'fabric.editor.ai.experience-application.documentInsertError': string;
    'fabric.editor.ai.experience-application.error-boundary.markdownErrorMessage': string;
    'fabric.editor.ai.experience-application.rateLimitEnforced': string;
    'fabric.editor.ai.experience-application.tokenLimitExceeded': string;
    'fabric.editor.ai.experience-application.unhandledErrorMessage': string;
    'fabric.editor.ai.experience.apiError': string;
    'fabric.editor.ai.experience.aupViolationMessage': string;
    'fabric.editor.ai.experience.browseMore.agents': string;
    'fabric.editor.ai.experience.discardMessagePrompt': string;
    'fabric.editor.ai.experience.discardMessageResponse': string;
    'fabric.editor.ai.experience.elevateDisabledGenetateError': string;
    'fabric.editor.ai.experience.engagementBannerLinksText': string;
    'fabric.editor.ai.experience.footerRightText.agents': string;
    'fabric.editor.ai.experience.hipaaContentError': string;
    'fabric.editor.ai.experience.inputTooShortError': string;
    'fabric.editor.ai.experience.internalServerError': string;
    'fabric.editor.ai.experience.labelRight.atlassianIntelligence': string;
    'fabric.editor.ai.experience.labelRight.rovo': string;
    'fabric.editor.ai.experience.loadingTitle': string;
    'fabric.editor.ai.experience.messageTemplateAnalysingQuery': string;
    'fabric.editor.ai.experience.messageTemplateContentSearch': string;
    'fabric.editor.ai.experience.messageTemplateNextBestTask': string;
    'fabric.editor.ai.experience.messageTemplatePageHydration': string;
    'fabric.editor.ai.experience.noResultsSuggestion': string;
    'fabric.editor.ai.experience.responseTooSimilarMessage': string;
    'fabric.editor.ai.experience.showMoreButton': string;
    'fabric.editor.ai.experience.suggestionGroupHeading.actions': string;
    'fabric.editor.ai.experience.suggestionGroupHeading.agents': string;
    'fabric.editor.ai.experience.suggestionGroupHeading.atlassianIntelligence': string;
    'fabric.editor.ai.experience.suggestionGroupHeading.knowledgeSource': string;
    'fabric.editor.ai.experience.suggestionGroupHeading.rovoAgents': string;
    'fabric.editor.ai.experience.suggestionGroupHeading.rovoAgentsCount': string;
    'fabric.editor.ai.experience.suggestionGroupHeading.useRovoAgentsCount': string;
    'fabric.editor.ai.experience.useGeneralAiKnowledgeLabel': string;
    'fabric.editor.ai.experience.useRovoOrganisationKnowledgeLabel': string;
    'fabric.editor.ai.proactive.askAIButtonLabel.non-final': string;
    'fabric.editor.ai.proactive.beta.label.non-final': string;
    'fabric.editor.ai.proactive.close.label.non-final': string;
    'fabric.editor.ai.proactive.enableSuggestionsButtonLabel.non-final': string;
    'fabric.editor.ai.proactive.enableSuggestionsState.description.non-final': string;
    'fabric.editor.ai.proactive.enableSuggestionsState.title.non-final': string;
    'fabric.editor.ai.proactive.error.markdownErrorMessage': string;
    'fabric.editor.ai.proactive.error.unhandledErrorMessage': string;
    'fabric.editor.ai.proactive.findingMoreSuggestionsLoadingLabel.non-final': string;
    'fabric.editor.ai.proactive.findingSuggestionsLabel.non-final': string;
    'fabric.editor.ai.proactive.findingSuggestionsLoadingLabel.non-final': string;
    'fabric.editor.ai.proactive.loading.iconAltText.non-final': string;
    'fabric.editor.ai.proactive.moreActions.label.non-final': string;
    'fabric.editor.ai.proactive.moreMenu.giveFeedback.non-final': string;
    'fabric.editor.ai.proactive.moreMenu.label.non-final': string;
    'fabric.editor.ai.proactive.moreMenu.manageSuggestions.non-final': string;
    'fabric.editor.ai.proactive.moreMenu.manageSuggestionsTitle.non-final': string;
    'fabric.editor.ai.proactive.moreMenu.toggleAllPanelSuggestionsDescription.non-final': string;
    'fabric.editor.ai.proactive.moreMenu.toggleAllPanelSuggestionsTitle.non-final': string;
    'fabric.editor.ai.proactive.moreMenu.toggleAllSuggestionsDescription.non-final': string;
    'fabric.editor.ai.proactive.moreMenu.toggleAllSuggestionsTitle.non-final': string;
    'fabric.editor.ai.proactive.moreMenu.toggleErrorPanelDescription.non-final': string;
    'fabric.editor.ai.proactive.moreMenu.toggleErrorPanelTitle.non-final': string;
    'fabric.editor.ai.proactive.moreMenu.toggleFormatParagraphDescription.non-final': string;
    'fabric.editor.ai.proactive.moreMenu.toggleFormatParagraphTitle.non-final': string;
    'fabric.editor.ai.proactive.moreMenu.toggleInfoPanelDescription.non-final': string;
    'fabric.editor.ai.proactive.moreMenu.toggleInfoPanelTitle.non-final': string;
    'fabric.editor.ai.proactive.moreMenu.toggleNotePanelDescription.non-final': string;
    'fabric.editor.ai.proactive.moreMenu.toggleNotePanelTitle.non-final': string;
    'fabric.editor.ai.proactive.moreMenu.toggleSplitParagraphDescription.non-final': string;
    'fabric.editor.ai.proactive.moreMenu.toggleSplitParagraphTitle.non-final': string;
    'fabric.editor.ai.proactive.moreMenu.toggleSuccessPanelDescription.non-final': string;
    'fabric.editor.ai.proactive.moreMenu.toggleSuccessPanelTitle.non-final': string;
    'fabric.editor.ai.proactive.moreMenu.toggleTableDescription.non-final': string;
    'fabric.editor.ai.proactive.moreMenu.toggleTableTitle.non-final': string;
    'fabric.editor.ai.proactive.moreMenu.toggleWarningPanelDescription.non-final': string;
    'fabric.editor.ai.proactive.moreMenu.toggleWarningPanelTitle.non-final': string;
    'fabric.editor.ai.proactive.noSuggestionsState.description.non-final': string;
    'fabric.editor.ai.proactive.noSuggestionsState.title.non-final': string;
    'fabric.editor.ai.proactive.preview.dismissProactiveRecommendation.non-final': string;
    'fabric.editor.ai.proactive.preview.insertBelowProactiveRecommendation': string;
    'fabric.editor.ai.proactive.preview.replaceProactiveRecommendation': string;
    'fabric.editor.ai.proactive.recommendation.convertToErrorPanel.description.non-final': string;
    'fabric.editor.ai.proactive.recommendation.convertToErrorPanel.title.non-final': string;
    'fabric.editor.ai.proactive.recommendation.convertToInfoPanel.description.non-final': string;
    'fabric.editor.ai.proactive.recommendation.convertToInfoPanel.title.non-final': string;
    'fabric.editor.ai.proactive.recommendation.convertToNotePanel.description.non-final': string;
    'fabric.editor.ai.proactive.recommendation.convertToNotePanel.title.non-final': string;
    'fabric.editor.ai.proactive.recommendation.convertToSuccessPanel.description.non-final': string;
    'fabric.editor.ai.proactive.recommendation.convertToSuccessPanel.title.non-final': string;
    'fabric.editor.ai.proactive.recommendation.convertToTable.description.non-final': string;
    'fabric.editor.ai.proactive.recommendation.convertToTable.title.non-final': string;
    'fabric.editor.ai.proactive.recommendation.convertToWarnPanel.description.non-final': string;
    'fabric.editor.ai.proactive.recommendation.convertToWarnPanel.title.non-final': string;
    'fabric.editor.ai.proactive.recommendation.default.description.non-final': string;
    'fabric.editor.ai.proactive.recommendation.default.title.non-final': string;
    'fabric.editor.ai.proactive.recommendation.dismiss.button.non-final': string;
    'fabric.editor.ai.proactive.recommendation.formatParagraph.description.non-final': string;
    'fabric.editor.ai.proactive.recommendation.formatParagraph.title.non-final': string;
    'fabric.editor.ai.proactive.recommendation.formatParagraphClarity.description.non-final': string;
    'fabric.editor.ai.proactive.recommendation.formatParagraphClarity.title.non-final': string;
    'fabric.editor.ai.proactive.recommendation.formatParagraphList.description.non-final': string;
    'fabric.editor.ai.proactive.recommendation.formatParagraphList.title.non-final': string;
    'fabric.editor.ai.proactive.recommendation.formatParagraphSpellcheck.description.non-final': string;
    'fabric.editor.ai.proactive.recommendation.formatParagraphSpellcheck.title.non-final': string;
    'fabric.editor.ai.proactive.recommendation.formatParagraphStructure.description.non-final': string;
    'fabric.editor.ai.proactive.recommendation.formatParagraphStructure.title.non-final': string;
    'fabric.editor.ai.proactive.recommendation.preview.button.non-final': string;
    'fabric.editor.ai.proactive.recommendation.replace.button.non-final': string;
    'fabric.editor.ai.proactive.recommendation.splitParagraph.description.non-final': string;
    'fabric.editor.ai.proactive.recommendation.splitParagraph.title.non-final': string;
    'fabric.editor.ai.proactive.reloadSuggestionsButtonLabel.non-final': string;
    'fabric.editor.ai.proactive.reloadSuggestionsState.description.non-final': string;
    'fabric.editor.ai.proactive.reloadSuggestionsState.title.non-final': string;
    'fabric.editor.ai.proactive.spellingAndGrammar.label.non-final': string;
    'fabric.editor.ai.proactive.suggestions.title.non-final': string;
    'fabric.editor.ai.screens.preview.convoAIBeta': string;
    'fabric.editor.ai.selectionToolbar.define.disabled.tooltip': string;
    'fabric.editor.ai.selectionToolbar.define.title': string;
    'fabric.editor.ai.toolbar.askAI.title': string;
    'fabric.editor.ai.toolbar.improveWriting.title': string;
    'fabric.editor.ai.toolbar.improveWriting.tooltip': string;
    'fabric.editor.ai.toolbar.simplifiedAI.title': string;
    'fabric.editor.ai.toolbar.titleText': string;
    'fabric.editor.ai.toolbar.tryAI.title': string;
    'fabric.editor.ai.toolbar.tryAI.tooltip': string;
    'fabric.editor.alignCenter': string;
    'fabric.editor.alignImageCenter': string;
    'fabric.editor.alignImageLeft': string;
    'fabric.editor.alignImageRight': string;
    'fabric.editor.alignLeft': string;
    'fabric.editor.alignRight': string;
    'fabric.editor.alignTableCenter': string;
    'fabric.editor.alignTableLeft': string;
    'fabric.editor.alignment': string;
    'fabric.editor.altText': string;
    'fabric.editor.alttext.validation': string;
    'fabric.editor.annotationToolbar': string;
    'fabric.editor.backLink': string;
    'fabric.editor.backgroundColor': string;
    'fabric.editor.blockCardUnavailable': string;
    'fabric.editor.blockControlDelete': string;
    'fabric.editor.blockControlDragHandleMove': string;
    'fabric.editor.blockControlDragHandleRearrange': string;
    'fabric.editor.blockControlInsert': string;
    'fabric.editor.blockControlMoveDown': string;
    'fabric.editor.blockControlMoveLeft': string;
    'fabric.editor.blockControlMoveRight': string;
    'fabric.editor.blockControlMoveUp': string;
    'fabric.editor.blockControlMovedDown': string;
    'fabric.editor.blockControlMovedUp': string;
    'fabric.editor.blockDescription': string;
    'fabric.editor.blockTitle': string;
    'fabric.editor.blockquote': string;
    'fabric.editor.blockquote.description': string;
    'fabric.editor.blockquote2': string;
    'fabric.editor.bold': string;
    'fabric.editor.bug': string;
    'fabric.editor.canNotSortTable': string;
    'fabric.editor.cancelButton': string;
    'fabric.editor.captionPlaceholder': string;
    'fabric.editor.captionPlaceholderWithDoubleClickPrompt': string;
    'fabric.editor.cardFloatingControls': string;
    'fabric.editor.cellBackground': string;
    'fabric.editor.cellOptions': string;
    'fabric.editor.checkUncheckActionItem': string;
    'fabric.editor.clearAltTextEdit': string;
    'fabric.editor.clearCells': string;
    'fabric.editor.clearFormatting': string;
    'fabric.editor.clearLink': string;
    'fabric.editor.clearLinkText': string;
    'fabric.editor.closeAltTextEdit': string;
    'fabric.editor.closeFindReplaceDialog': string;
    'fabric.editor.closeHelpDialog': string;
    'fabric.editor.code': string;
    'fabric.editor.code.on': string;
    'fabric.editor.codeBidiWarningLabel': string;
    'fabric.editor.codeBlockCopyButton.copiedToClipboard': string;
    'fabric.editor.codeBlockCopyButton.copyToClipboard': string;
    'fabric.editor.codeBlockWrapButton.unwrapCodeBlock': string;
    'fabric.editor.codeBlockWrapButton.wrapCodeBlock': string;
    'fabric.editor.codeblock': string;
    'fabric.editor.codeblock.description': string;
    'fabric.editor.collapseNode': string;
    'fabric.editor.collapseTable': string;
    'fabric.editor.columnDragHandle': string;
    'fabric.editor.columnMovedLeft': string;
    'fabric.editor.columnMovedRight': string;
    'fabric.editor.columnSelected': string;
    'fabric.editor.columns': string;
    'fabric.editor.columns.advanced.layout.description': string;
    'fabric.editor.columns.advanced.layout.single.description': string;
    'fabric.editor.columns.description': string;
    'fabric.editor.columnsAreInserted': string;
    'fabric.editor.columnsAreRemoved': string;
    'fabric.editor.configFailedToLoad': string;
    'fabric.editor.configPanel.cancel': string;
    'fabric.editor.configPanel.close': string;
    'fabric.editor.configPanel.customSelect.createOption': string;
    'fabric.editor.configPanel.dateRange.custom.from': string;
    'fabric.editor.configPanel.dateRange.custom.to': string;
    'fabric.editor.configPanel.dateRange.grouping.collapse': string;
    'fabric.editor.configPanel.dateRange.grouping.expand': string;
    'fabric.editor.configPanel.dateRange.option.custom': string;
    'fabric.editor.configPanel.documentation': string;
    'fabric.editor.configPanel.errorBoundary.note': string;
    'fabric.editor.configPanel.errorBoundary.title': string;
    'fabric.editor.configPanel.fieldTypeError.isMultipleAndRadio': string;
    'fabric.editor.configPanel.formType.addField': string;
    'fabric.editor.configPanel.formType.removeField': string;
    'fabric.editor.configPanel.help': string;
    'fabric.editor.configPanel.invalid': string;
    'fabric.editor.configPanel.required': string;
    'fabric.editor.configPanel.submit': string;
    'fabric.editor.copiedToClipboard': string;
    'fabric.editor.copyToClipboard': string;
    'fabric.editor.cornerControl': string;
    'fabric.editor.createComment': string;
    'fabric.editor.createCommentDisabled': string;
    'fabric.editor.createCommentInvalid': string;
    'fabric.editor.createCommentOnInlineNodeSpotlight.action': string;
    'fabric.editor.createCommentOnInlineNodeSpotlight.body': string;
    'fabric.editor.customPanel': string;
    'fabric.editor.customPanel.description': string;
    'fabric.editor.datasource.assetsObjects.description': string;
    'fabric.editor.datasource.assetsObjectsGeneralAvailability': string;
    'fabric.editor.datasource.confluenceSearch': string;
    'fabric.editor.datasource.confluenceSearch.description': string;
    'fabric.editor.datasource.jiraIssue': string;
    'fabric.editor.datasource.jiraIssue.description': string;
    'fabric.editor.datasource.jiraIssue.description.issue-term-refresh': string;
    'fabric.editor.datasource.jiraIssue.issue-term-refresh': string;
    'fabric.editor.datasourceAppearanceTitle': string;
    'fabric.editor.datasourceTitle': string;
    'fabric.editor.date': string;
    'fabric.editor.date.description': string;
    'fabric.editor.datePickerOpenedAlert': string;
    'fabric.editor.decision': string;
    'fabric.editor.decision.description': string;
    'fabric.editor.decisionPlaceholder': string;
    'fabric.editor.decreaseSize': string;
    'fabric.editor.defaultAltText': string;
    'fabric.editor.delete': string;
    'fabric.editor.description': string;
    'fabric.editor.displayBlock': string;
    'fabric.editor.displayEmbed': string;
    'fabric.editor.displayInline': string;
    'fabric.editor.displayLink': string;
    'fabric.editor.displayOptionUnavailableInParentNode': string;
    'fabric.editor.displayText': string;
    'fabric.editor.distributeColumns': string;
    'fabric.editor.dropPlaceholderLabel': string;
    'fabric.editor.ecombedDescription': string;
    'fabric.editor.edit': string;
    'fabric.editor.edit.datasource': string;
    'fabric.editor.edit.searchInput': string;
    'fabric.editor.editAccessTitle': string;
    'fabric.editor.editAltText': string;
    'fabric.editor.editDatasourceStandalone': string;
    'fabric.editor.editDatasourceStandaloneTooltip': string;
    'fabric.editor.editDateText': string;
    'fabric.editor.editDropdownEditDatasourceTitle': string;
    'fabric.editor.editDropdownEditLinkTitle': string;
    'fabric.editor.editDropdownExpandIconLabel': string;
    'fabric.editor.editDropdownTriggerTitle': string;
    'fabric.editor.editLink': string;
    'fabric.editor.editMode.inviteToEditButton.title': string;
    'fabric.editor.editStatusColor': string;
    'fabric.editor.editStatusText': string;
    'fabric.editor.editableContentLabel': string;
    'fabric.editor.editorHelp': string;
    'fabric.editor.editors': string;
    'fabric.editor.elementBrowser.help': string;
    'fabric.editor.elementInsertSidePanel.title': string;
    'fabric.editor.elementbrowser.categorylist.category-admin': string;
    'fabric.editor.elementbrowser.categorylist.category-ai': string;
    'fabric.editor.elementbrowser.categorylist.category-all': string;
    'fabric.editor.elementbrowser.categorylist.category-communication': string;
    'fabric.editor.elementbrowser.categorylist.category-confluence-content': string;
    'fabric.editor.elementbrowser.categorylist.category-development': string;
    'fabric.editor.elementbrowser.categorylist.category-external-content': string;
    'fabric.editor.elementbrowser.categorylist.category-formatting': string;
    'fabric.editor.elementbrowser.categorylist.category-media': string;
    'fabric.editor.elementbrowser.categorylist.category-navigation': string;
    'fabric.editor.elementbrowser.categorylist.category-reporting': string;
    'fabric.editor.elementbrowser.categorylist.category-visuals': string;
    'fabric.editor.elementbrowser.search.empty-state.heading': string;
    'fabric.editor.elementbrowser.search.empty-state.sub-heading': string;
    'fabric.editor.elementbrowser.search.empty-state.sub-heading.link': string;
    'fabric.editor.elementbrowser.searchbar.assistive.text.default': string;
    'fabric.editor.elementbrowser.searchbar.assistive.text.result': string;
    'fabric.editor.elementbrowser.searchbar.elementAfterInput': string;
    'fabric.editor.elementbrowser.searchbar.placeholder': string;
    'fabric.editor.elementbrowser.sidebar.heading': string;
    'fabric.editor.embedTitle': string;
    'fabric.editor.emoji': string;
    'fabric.editor.emoji.description': string;
    'fabric.editor.emptyLink': string;
    'fabric.editor.emptySearchResults': string;
    'fabric.editor.emptySearchResultsSuggestion': string;
    'fabric.editor.epic': string;
    'fabric.editor.error': string;
    'fabric.editor.errorPanel': string;
    'fabric.editor.errorPanel.description': string;
    'fabric.editor.expand': string;
    'fabric.editor.expand.description': string;
    'fabric.editor.expandAriaLabel': string;
    'fabric.editor.expandDefaultTitle': string;
    'fabric.editor.expandNode': string;
    'fabric.editor.expandPlaceholder': string;
    'fabric.editor.extension.confirmDeleteLinkedModalMessage': string;
    'fabric.editor.extension.confirmDeleteLinkedModalMessagePrefix': string;
    'fabric.editor.extension.confirmDeleteLinkedModalOKButton': string;
    'fabric.editor.extension.deleteElementTitle': string;
    'fabric.editor.extension.sourceNoTitledName': string;
    'fabric.editor.extensions.config-panel.save-indicator': string;
    'fabric.editor.externalExtensionsHeading': string;
    'fabric.editor.externalMediaFile': string;
    'fabric.editor.failed_to_upload': string;
    'fabric.editor.feedbackDialog': string;
    'fabric.editor.feedbackDialog.description': string;
    'fabric.editor.find': string;
    'fabric.editor.findNext': string;
    'fabric.editor.findPrevious': string;
    'fabric.editor.findReplaceToolbarButton': string;
    'fabric.editor.fiveColumns': string;
    'fabric.editor.floatingToolbar.confirmModalCancel': string;
    'fabric.editor.floatingToolbar.confirmModalCheckboxLabel': string;
    'fabric.editor.floatingToolbar.confirmModalConnectedUnit': string;
    'fabric.editor.floatingToolbar.confirmModalHeading': string;
    'fabric.editor.floatingToolbar.confirmModalOK': string;
    'fabric.editor.floatingToolbar.floatingToolbarAnnouncer': string;
    'fabric.editor.floatingToolbar.floatingToolbarAriaLabel': string;
    'fabric.editor.floatingToolbar.floatingToolbarRadioGroupAriaLabel': string;
    'fabric.editor.floatingToolbar.scrollLeft': string;
    'fabric.editor.floatingToolbar.scrollRight': string;
    'fabric.editor.focusTableResizeHandle': string;
    'fabric.editor.fourColumns': string;
    'fabric.editor.headerColumn': string;
    'fabric.editor.headerRow': string;
    'fabric.editor.heading1': string;
    'fabric.editor.heading1Description': string;
    'fabric.editor.heading2': string;
    'fabric.editor.heading2Description': string;
    'fabric.editor.heading3': string;
    'fabric.editor.heading3Description': string;
    'fabric.editor.heading4': string;
    'fabric.editor.heading4Description': string;
    'fabric.editor.heading5': string;
    'fabric.editor.heading5Description': string;
    'fabric.editor.heading6': string;
    'fabric.editor.heading6Description': string;
    'fabric.editor.headingLink.ascOrderLabel': string;
    'fabric.editor.headingLink.descOrderLabel': string;
    'fabric.editor.headingLink.hyperlinkIconBlogLabel': string;
    'fabric.editor.headingLink.hyperlinkIconBugLabel': string;
    'fabric.editor.headingLink.hyperlinkIconIssueLabel': string;
    'fabric.editor.headingLink.hyperlinkIconIssueLabel.issue-term-refresh': string;
    'fabric.editor.headingLink.hyperlinkIconPageLabel': string;
    'fabric.editor.headingLink.hyperlinkIconStoryLabel': string;
    'fabric.editor.headingLink.hyperlinkIconTaskLabel': string;
    'fabric.editor.headingLink.invalidLabel': string;
    'fabric.editor.headingLink.noOrderLabel': string;
    'fabric.editor.help': string;
    'fabric.editor.help.description': string;
    'fabric.editor.helpDialogTips': string;
    'fabric.editor.highlight': string;
    'fabric.editor.highlightColor': string;
    'fabric.editor.highlightFloatingToolbar': string;
    'fabric.editor.horizontalRule': string;
    'fabric.editor.horizontalRule.description': string;
    'fabric.editor.hyperlink.linkVisibleLabel': string;
    'fabric.editor.hyperlink.searchLinkAriaDescription': string;
    'fabric.editor.hyperlink.searchLinkResults': string;
    'fabric.editor.hyperlink.textVisibleLabel': string;
    'fabric.editor.hyperlinkToolbarPlaceholder': string;
    'fabric.editor.image': string;
    'fabric.editor.image.fullWidthLabel': string;
    'fabric.editor.imageBorderBold': string;
    'fabric.editor.imageBorderColor': string;
    'fabric.editor.imageBorderMedium': string;
    'fabric.editor.imageBorderOptions': string;
    'fabric.editor.imageBorderSize': string;
    'fabric.editor.imageBorderSubtle': string;
    'fabric.editor.improvement': string;
    'fabric.editor.increaseSize': string;
    'fabric.editor.indent': string;
    'fabric.editor.info': string;
    'fabric.editor.infoPanel': string;
    'fabric.editor.infoPanel.description': string;
    'fabric.editor.inlineConfigureLink': string;
    'fabric.editor.inlineDescription': string;
    'fabric.editor.inlineGoToLink': string;
    'fabric.editor.inlineOverlay': string;
    'fabric.editor.inlineTitle': string;
    'fabric.editor.inputQueryAssistiveTxt': string;
    'fabric.editor.insertColumn': string;
    'fabric.editor.insertColumnDrag': string;
    'fabric.editor.insertMenu': string;
    'fabric.editor.insertRightRail.close': string;
    'fabric.editor.insertRightRail.title': string;
    'fabric.editor.insertRow': string;
    'fabric.editor.insertRowDrag': string;
    'fabric.editor.insertTableColumn': string;
    'fabric.editor.insertTableRow': string;
    'fabric.editor.invalidDateError': string;
    'fabric.editor.invalidLink': string;
    'fabric.editor.inviteItem.title': string;
    'fabric.editor.italic': string;
    'fabric.editor.keyboardShortcuts': string;
    'fabric.editor.layout.columnOption': string;
    'fabric.editor.layout.controls.placeholder': string;
    'fabric.editor.layout.placeholder': string;
    'fabric.editor.layout.resizeLayout': string;
    'fabric.editor.layoutFixedWidth': string;
    'fabric.editor.layoutFullWidth': string;
    'fabric.editor.layoutStateFixedWidth': string;
    'fabric.editor.layoutStateFullWidth': string;
    'fabric.editor.layoutStateWide': string;
    'fabric.editor.layoutWide': string;
    'fabric.editor.leftSidebar': string;
    'fabric.editor.link': string;
    'fabric.editor.link.description': string;
    'fabric.editor.linkAddress': string;
    'fabric.editor.linkPlaceholder': string;
    'fabric.editor.lists': string;
    'fabric.editor.listsFormat': string;
    'fabric.editor.lockColumns': string;
    'fabric.editor.longEmptyNodePlaceholderText': string;
    'fabric.editor.markdown': string;
    'fabric.editor.matchCase': string;
    'fabric.editor.media.DefaultMediaWidthDecreased': string;
    'fabric.editor.media.DefaultMediaWidthIncreased': string;
    'fabric.editor.media.MediaWidthIsMin': string;
    'fabric.editor.media.insert.cancel': string;
    'fabric.editor.media.insert.fileTabTitle': string;
    'fabric.editor.media.insert.fromUrlErrorMessage': string;
    'fabric.editor.media.insert.fromUrlWarning': string;
    'fabric.editor.media.insert.insert': string;
    'fabric.editor.media.insert.invalidUrlErrorMessage': string;
    'fabric.editor.media.insert.linkTabTitle': string;
    'fabric.editor.media.insert.loadPreview': string;
    'fabric.editor.media.insert.localFileErrorMessage': string;
    'fabric.editor.media.insert.localFileNetworkErrorMessage': string;
    'fabric.editor.media.insert.mediaAlt': string;
    'fabric.editor.media.insert.mediaPickerPopupAriaLabel': string;
    'fabric.editor.media.insert.pasteLinkToUpload': string;
    'fabric.editor.media.insert.upload': string;
    'fabric.editor.media.pixelEntry.MediaWidthIsMax': string;
    'fabric.editor.media.pixelEntry.ResizeOption': string;
    'fabric.editor.media.pixelEntry.closePixelEntry': string;
    'fabric.editor.media.pixelEntry.inputHeightAriaLabel': string;
    'fabric.editor.media.pixelEntry.inputHeightTooltip': string;
    'fabric.editor.media.pixelEntry.inputWidthAriaLabel': string;
    'fabric.editor.media.pixelEntry.inputWidthLabel': string;
    'fabric.editor.media.pixelEntry.inputWidthTooltip': string;
    'fabric.editor.media.pixelEntry.migrationButtonText': string;
    'fabric.editor.media.pixelEntry.migrationButtonTooltip': string;
    'fabric.editor.media.pixelEntry.submitButtonText': string;
    'fabric.editor.media.pixelEntry.validationFailedMessage': string;
    'fabric.editor.mediaAddLink': string;
    'fabric.editor.mediaFiles': string;
    'fabric.editor.mediaFiles.description': string;
    'fabric.editor.mediaGroupDeleteLabel': string;
    'fabric.editor.media_change_mediainline_caption_warn': string;
    'fabric.editor.media_change_mediainline_default': string;
    'fabric.editor.media_change_mediasingle': string;
    'fabric.editor.mention': string;
    'fabric.editor.mention.description': string;
    'fabric.editor.mentionNode.label': string;
    'fabric.editor.mentionsAddLabel': string;
    'fabric.editor.mentionsIconLabel': string;
    'fabric.editor.mergeCells': string;
    'fabric.editor.moreFormatting': string;
    'fabric.editor.moveColumnLeft': string;
    'fabric.editor.moveColumnRight': string;
    'fabric.editor.moveDownDown': string;
    'fabric.editor.moveRowUp': string;
    'fabric.editor.navigate.toolbar.editor': string;
    'fabric.editor.navigate.toolbar.floating': string;
    'fabric.editor.noResultsFound': string;
    'fabric.editor.noSearchResults': string;
    'fabric.editor.normal': string;
    'fabric.editor.note': string;
    'fabric.editor.notePanel': string;
    'fabric.editor.notePanel.description': string;
    'fabric.editor.numberedColumn': string;
    'fabric.editor.off': string;
    'fabric.editor.offlineErrorFallbackDescription': string;
    'fabric.editor.offlineErrorFallbackHeading': string;
    'fabric.editor.on': string;
    'fabric.editor.openCellOptions': string;
    'fabric.editor.openLink': string;
    'fabric.editor.orderedList': string;
    'fabric.editor.orderedList.description': string;
    'fabric.editor.other': string;
    'fabric.editor.outdent': string;
    'fabric.editor.overflowMenuViewMore': string;
    'fabric.editor.pageActionsLabel': string;
    'fabric.editor.panel': string;
    'fabric.editor.panel.backgroundColor': string;
    'fabric.editor.panel.custom': string;
    'fabric.editor.panel.emoji': string;
    'fabric.editor.panel.panelsGroup': string;
    'fabric.editor.pasteOptions': string;
    'fabric.editor.pastePlainText': string;
    'fabric.editor.placeholderAltText': string;
    'fabric.editor.placeholderText': string;
    'fabric.editor.placeholderText.description': string;
    'fabric.editor.placeholderTextPlaceholder': string;
    'fabric.editor.plainText': string;
    'fabric.editor.preferencesLink': string;
    'fabric.editor.quickInsert': string;
    'fabric.editor.recordLoomShortTitle': string;
    'fabric.editor.recordVideo': string;
    'fabric.editor.recordVideo.description': string;
    'fabric.editor.redo': string;
    'fabric.editor.remove': string;
    'fabric.editor.removeColumns': string;
    'fabric.editor.removeEmoji': string;
    'fabric.editor.removeImageBorder': string;
    'fabric.editor.removeRows': string;
    'fabric.editor.replace': string;
    'fabric.editor.replaceAll': string;
    'fabric.editor.replaceSuccess': string;
    'fabric.editor.replaceWith': string;
    'fabric.editor.requestToEdit': string;
    'fabric.editor.requestToEditDescription': string;
    'fabric.editor.requestToEditDismiss': string;
    'fabric.editor.resultsCount': string;
    'fabric.editor.richText': string;
    'fabric.editor.rightSidebar': string;
    'fabric.editor.rowControl': string;
    'fabric.editor.rowDragHandle': string;
    'fabric.editor.rowMovedDown': string;
    'fabric.editor.rowMovedUp': string;
    'fabric.editor.rowSelected': string;
    'fabric.editor.rowsAreInserted': string;
    'fabric.editor.rowsAreRemoved': string;
    'fabric.editor.saveButton': string;
    'fabric.editor.searchResults': string;
    'fabric.editor.selectColumnResize': string;
    'fabric.editor.selectLanguage': string;
    'fabric.editor.selectTableColumn': string;
    'fabric.editor.selectTableRow': string;
    'fabric.editor.selectionExtensionDropdownButtonLabel': string;
    'fabric.editor.settingsLinks': string;
    'fabric.editor.shortEmptyNodePlaceholderText': string;
    'fabric.editor.shortcut': string;
    'fabric.editor.single': string;
    'fabric.editor.singleColumns': string;
    'fabric.editor.sortColumnASC': string;
    'fabric.editor.sortColumnDESC': string;
    'fabric.editor.sortColumnDecreasing': string;
    'fabric.editor.sortColumnIncreasing': string;
    'fabric.editor.splitCell': string;
    'fabric.editor.status': string;
    'fabric.editor.status.description': string;
    'fabric.editor.statusPlaceholder': string;
    'fabric.editor.story': string;
    'fabric.editor.strike': string;
    'fabric.editor.subTask': string;
    'fabric.editor.subscript': string;
    'fabric.editor.subscript.off.superscript.on': string;
    'fabric.editor.success': string;
    'fabric.editor.successPanel': string;
    'fabric.editor.successPanel.description': string;
    'fabric.editor.superscript': string;
    'fabric.editor.superscript.off.subscript.on': string;
    'fabric.editor.supportAltText': string;
    'fabric.editor.table': string;
    'fabric.editor.table.description': string;
    'fabric.editor.tableAlignmentOptions': string;
    'fabric.editor.tableFullWidthLabel': string;
    'fabric.editor.tableHeader.sorting.asc': string;
    'fabric.editor.tableHeader.sorting.default': string;
    'fabric.editor.tableHeader.sorting.desc': string;
    'fabric.editor.tableHeader.sorting.invalid': string;
    'fabric.editor.tableHeader.sorting.no': string;
    'fabric.editor.tableOptions': string;
    'fabric.editor.tableResizeDecreaseScreenReaderInformation': string;
    'fabric.editor.tableResizeIncreaseScreenReaderInformation': string;
    'fabric.editor.tableSelector': string;
    'fabric.editor.tableSelector.description': string;
    'fabric.editor.tableSizeSelectorButton': string;
    'fabric.editor.tableSizeSelectorPopup': string;
    'fabric.editor.tables.adjustColumn': string;
    'fabric.editor.tables.columnResizeLast': string;
    'fabric.editor.tables.columnResizeOverflow': string;
    'fabric.editor.tables.columnWidth': string;
    'fabric.editor.tables.confirmDeleteLinkedModalMessage': string;
    'fabric.editor.tables.confirmDeleteLinkedModalOKButton': string;
    'fabric.editor.tables.otherResize': string;
    'fabric.editor.tables.resizeStop': string;
    'fabric.editor.tables.resizeTable': string;
    'fabric.editor.tables.startedColumnResize': string;
    'fabric.editor.task': string;
    'fabric.editor.taskPlaceholder': string;
    'fabric.editor.text.formatting.off': string;
    'fabric.editor.textColor': string;
    'fabric.editor.textColorTooltip': string;
    'fabric.editor.textFormat': string;
    'fabric.editor.textFormatting': string;
    'fabric.editor.textStyles': string;
    'fabric.editor.textStylesTooltip': string;
    'fabric.editor.threeColumns': string;
    'fabric.editor.threeColumnsWithLeftSidebars': string;
    'fabric.editor.threeColumnsWithRightSidebars': string;
    'fabric.editor.threeColumnsWithSidebars': string;
    'fabric.editor.time.ago': string;
    'fabric.editor.time.updated': string;
    'fabric.editor.time.viewed': string;
    'fabric.editor.toolbarAppears': string;
    'fabric.editor.toolbarLabel': string;
    'fabric.editor.toolbarMediaTitle': string;
    'fabric.editor.toolbarPositionFixedAtTop': string;
    'fabric.editor.toolbarPositionInline': string;
    'fabric.editor.tooltip.blockPanel': string;
    'fabric.editor.tooltip.bulletList': string;
    'fabric.editor.tooltip.decisionList': string;
    'fabric.editor.tooltip.defaultBlockNode': string;
    'fabric.editor.tooltip.nestedExpand': string;
    'fabric.editor.tooltip.taskList': string;
    'fabric.editor.turnInto': string;
    'fabric.editor.twoColumns': string;
    'fabric.editor.typeAhead.popupLabel': string;
    'fabric.editor.typeAhead.quickInsertInputLabel': string;
    'fabric.editor.typeAhead.quickInsertInputPlaceholderLabel': string;
    'fabric.editor.typeAhead.quickInsertPopupLabel': string;
    'fabric.editor.typeahead.emojiInputLabel': string;
    'fabric.editor.typeahead.emojiListItemLabel': string;
    'fabric.editor.typeahead.emojiPopupLabel': string;
    'fabric.editor.typeahead.mentionInputLabel': string;
    'fabric.editor.typeahead.mentionPopupLabel': string;
    'fabric.editor.typeahead.metionListItemLabel': string;
    'fabric.editor.unableToLoadContent': string;
    'fabric.editor.unableToOpenLink': string;
    'fabric.editor.underline': string;
    'fabric.editor.undo': string;
    'fabric.editor.unknown.label': string;
    'fabric.editor.unlink': string;
    'fabric.editor.unorderedList': string;
    'fabric.editor.unorderedList.description': string;
    'fabric.editor.unsupportedBlockContent': string;
    'fabric.editor.unsupportedContentTooltip': string;
    'fabric.editor.unsupportedInlineContent': string;
    'fabric.editor.upDown': string;
    'fabric.editor.url': string;
    'fabric.editor.urlDescription': string;
    'fabric.editor.urlTitle': string;
    'fabric.editor.useMarkdown': string;
    'fabric.editor.viewAndAddCommentsOnMedia': string;
    'fabric.editor.viewCommentOnMedia': string;
    'fabric.editor.viewMore': string;
    'fabric.editor.visit': string;
    'fabric.editor.warning': string;
    'fabric.editor.warningPanel': string;
    'fabric.editor.warningPanel.description': string;
    'fabric.editor.wrapLeft': string;
    'fabric.editor.wrapRight': string;
    'fabric.emoji.label': string;
    'fabric.theme.blue': string;
    'fabric.theme.bold-blue': string;
    'fabric.theme.bold-gray': string;
    'fabric.theme.bold-green': string;
    'fabric.theme.bold-magenta': string;
    'fabric.theme.bold-orange': string;
    'fabric.theme.bold-purple': string;
    'fabric.theme.bold-red': string;
    'fabric.theme.bold-teal': string;
    'fabric.theme.bold-yellow': string;
    'fabric.theme.bolder-blue': string;
    'fabric.theme.bolder-gray': string;
    'fabric.theme.bolder-green': string;
    'fabric.theme.bolder-magenta': string;
    'fabric.theme.bolder-orange': string;
    'fabric.theme.bolder-purple': string;
    'fabric.theme.bolder-red': string;
    'fabric.theme.bolder-teal': string;
    'fabric.theme.bolder-yellow': string;
    'fabric.theme.dark-blue': string;
    'fabric.theme.dark-gray': string;
    'fabric.theme.dark-green': string;
    'fabric.theme.dark-purple': string;
    'fabric.theme.dark-red': string;
    'fabric.theme.dark-teal': string;
    'fabric.theme.dark-yellow': string;
    'fabric.theme.default': string;
    'fabric.theme.gray': string;
    'fabric.theme.green': string;
    'fabric.theme.light-blue': string;
    'fabric.theme.light-gray': string;
    'fabric.theme.light-green': string;
    'fabric.theme.light-purple': string;
    'fabric.theme.light-red': string;
    'fabric.theme.light-teal': string;
    'fabric.theme.light-yellow': string;
    'fabric.theme.lime': string;
    'fabric.theme.magenta': string;
    'fabric.theme.no-color': string;
    'fabric.theme.orange': string;
    'fabric.theme.purple': string;
    'fabric.theme.red': string;
    'fabric.theme.subtle-blue': string;
    'fabric.theme.subtle-gray': string;
    'fabric.theme.subtle-green': string;
    'fabric.theme.subtle-magenta': string;
    'fabric.theme.subtle-orange': string;
    'fabric.theme.subtle-purple': string;
    'fabric.theme.subtle-red': string;
    'fabric.theme.subtle-teal': string;
    'fabric.theme.subtle-yellow': string;
    'fabric.theme.teal': string;
    'fabric.theme.white': string;
    'fabric.theme.yellow': string;
    'fabrid.editor.rowNumbers': string;
    'rovo.agents.dropdown.title': string;
    'rovo.agents.dropdown.viewAgents.label': string;
};
export default _default;
