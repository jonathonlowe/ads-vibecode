...
const JQLEditorView = ({
  ...
  const editorViewNodeRef = useRef(null);
  ...
  const onMainRef = useCallback(element => portalActions.onRegisterPluginContainer('main', element), [portalActions]);
  const onEditorViewContainerRef = useCallback(editorViewContainer => {
    if (editorViewContainer) {
      setEditorViewContainer(editorViewContainer);
    }
  }, [setEditorViewContainer]);
  const throttledSetEditorViewContainerScroll = useMemo(() => throttle(scrollTop => {
    setEditorViewContainerScroll(scrollTop);
  }, 1000 / 60),
  // 60 fps
  [setEditorViewContainerScroll]);
  const onEditorViewContainerScroll = useCallback(event => {
    const {
      scrollTop
    } = event.currentTarget;
  ...
  const onEditorViewRef = useCallback(editorViewNode => {
