...
const getKeyHandler = (keymap, key) => (state, dispatch, view) => {
  const binding = keymap.getKeyBinding(key);
  return binding ? binding(state, dispatch, view) : false;
};
const autocompletePlugin = (portalActions, enableRichInlineNodes) => {
  ...
  return new Plugin({
    ...
    view: editorView => {
      const view = new AutocompletePluginView(editorView, keymap, portalActions, enableRichInlineNodes);
