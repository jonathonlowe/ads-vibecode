 ...
 */
export const extractTitle = (jsonLd, removeTextHighlightingFromTitle) => {
  var _jsonLd$name;
  const name = (_jsonLd$name = jsonLd.name) === null || _jsonLd$name === void 0 ? void 0 : _jsonLd$name.replace(/[\r\n]+/g, '');
  const id = jsonLd['@id'] || '';
  ...
  const hasContextRepo = hasContextType && context.type.includes('atlassian:SourceCodeRepository');
  ...
  if (removeTextHighlightingFromTitle) {
    ...
    const truncated = name === null || name === void 0 ? void 0 : name.replace(textFragmentRegex, '');
    return truncated;
  }
  return name;
};