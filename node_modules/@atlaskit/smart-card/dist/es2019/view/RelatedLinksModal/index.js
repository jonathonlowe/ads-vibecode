...
const isGrantedResponse = response => {
...
const RelatedLinksModal = ({
  ...
  } = useIncomingOutgoingAri(baseUriWithNoTrailingSlash);
  const {
    connections
  } = useSmartLinkContext();
  const {
   ...
   */
  const resolveAris = useCallback(async aris => {
    if (aris.length === 0) {
      return [];
    }
    const urlArray = [];
  ...
  const fetchIncomingOutgoingData = useCallback(async () => {
    ...
    const [incomingUrls, outgoingUrls] = await Promise.all([resolveAris(incomingAris).catch(() => {
  ...
  useEffect(() => {
    ...
    fetchIncomingOutgoingData().then(({
      incomingUrls,
      outgoingUrls
    }) => {
  ...
  const renderView = () => {
    switch (modalStatus) {
      case 'error':
        return /*#__PURE__*/React.createElement(RelatedLinksErroredView, null);
      case 'resolved':
        return /*#__PURE__*/React.createElement(RelatedLinksResolvedView, {
          incomingLinks: incomingLinks,
          outgoingLinks: outgoingLinks
        });
      case 'unavailable':
        return /*#__PURE__*/React.createElement(RelatedLinksUnavailableView, null);
      default:
        return /*#__PURE__*/React.createElement(RelatedLinksResolvingView, null);
    }
  };
