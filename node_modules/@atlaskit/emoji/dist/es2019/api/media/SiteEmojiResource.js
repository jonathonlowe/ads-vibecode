...
export default class SiteEmojiResource {
  ...
  constructor(siteServiceConfig, mediaApiToken) {
    _defineProperty(this, "postToEmojiService", (upload, fileId) => {
      const {
        shortName,
        name
      } = upload;
      ...
      return serviceUtils.requestService(this.siteServiceConfig, {
        ...
        if (emojis.length) {
          ...
          const imgAltRepresentation = altRepresentation ? convertImageToMediaRepresentation(altRepresentation) : undefined;
  ...
  optimisticRendering(emoji, useAlt) {
  ...
  uploadEmoji(upload, retry = false, progressCallback) {
   ...
   */
  hasUploadToken() {
    const tokenPromise = this.tokenManager.getToken('upload');
