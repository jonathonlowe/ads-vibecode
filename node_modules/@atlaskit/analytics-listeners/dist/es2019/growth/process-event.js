...
import { GROWTH_CONTEXT } from '@atlaskit/analytics-namespaced-context';
...
export default ((event, logger) => {
  const sources = getSources(event, GROWTH_CONTEXT);
  const source = last(sources) || DEFAULT_SOURCE;
  const extraAttributes = getExtraAttributes(event, GROWTH_CONTEXT);
  const components = getComponents(event, GROWTH_CONTEXT);
  const packages = getPackageInfo(event, GROWTH_CONTEXT);
  const {
    packageName,
    packageVersion
  } = last(getPackageInfo(event, GROWTH_CONTEXT)) || {};
