...
const extractFromEventContext = (propertyNames, event, namespacedContextOnly = true, contextName) => event.context.reduce((acc, contextItem) => {
  propertyNames.forEach(propertyName => {
    ...
    const value = namespacedContextOnly ? navContextProp : navContextProp || contextItem[propertyName];
    if (value) {
      acc.push(value);
    }
  });
...
export const getPackageInfo = (event, contextName) => event.context.map(contextItem => {
  const navContext = contextItem[contextName];
