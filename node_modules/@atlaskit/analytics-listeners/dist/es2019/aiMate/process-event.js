...
export default ((event, logger) => {
  const sources = getSources(event, AI_MATE_CONTEXT);
  const source = last(sources) || DEFAULT_SOURCE;
  const extraAttributes = getExtraAttributes(event, AI_MATE_CONTEXT);
  const components = getComponents(event, AI_MATE_CONTEXT);
  const packages = getPackageInfo(event, AI_MATE_CONTEXT);
  const {
    packageName,
    packageVersion
  } = last(getPackageInfo(event, AI_MATE_CONTEXT)) || {};
