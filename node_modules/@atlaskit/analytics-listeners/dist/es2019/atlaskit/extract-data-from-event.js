...
export const extractFromEventContext = (propertyName, event) => event.context.map(contextItem => contextItem[propertyName]).filter(Boolean);
...
export const getPackageInfo = event => event.context.map(contextItem => ({
...
export function getPackageHierarchy(event) {
  ...
  return packages.map(p => p.packageVersion ? `${p.packageName}@${p.packageVersion}` : p.packageName).join(',') || undefined;
}