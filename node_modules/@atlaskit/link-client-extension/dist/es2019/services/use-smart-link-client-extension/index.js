import { useCallback, useMemo } from 'react';
import { request } from '@atlaskit/linking-common';
import { InvokeError } from '@atlaskit/linking-types/smart-link-actions';
import { useResolverUrl } from '../use-resolver-url';

/**
 * useSmartLinkClientExtension
 *
 * This hook extends the usage of @atlaskit/link-provider CardClient
 * for smart links.
 *
 * @param cardClient
 */
export const useSmartLinkClientExtension = cardClient => {
  const resolverUrl = useResolverUrl(cardClient);
  const invoke = useCallback(async data => {
    try {
      return await request('post', `${resolverUrl}/invoke`, data, undefined, [200, 201, 202, 203, 204]);
    } catch (err) {
      // checking if the error is a Response type
      // since we already had a few HOTs caused by comparing with instanceOf',
      // we're verifying the type by the contents
      if (err && err.json && typeof err.json === 'function') {
        const errorBody = await err.json();
        if (errorBody) {
          throw new InvokeError(errorBody === null || errorBody === void 0 ? void 0 : errorBody.error.message, errorBody === null || errorBody === void 0 ? void 0 : errorBody.error.status);
        }
      }
      throw err;
    }
  }, [resolverUrl]);
  return useMemo(() => ({
    invoke
  }), [invoke]);
};