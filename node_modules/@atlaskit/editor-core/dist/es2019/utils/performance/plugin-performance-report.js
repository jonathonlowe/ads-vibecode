...
export class PluginPerformanceReport {
  constructor(entry) {
    ...
    _defineProperty(this, "isChild", child => child.startTime >= this.entry.startTime && child.startTime + child.duration <= this.entry.startTime + this.entry.duration);
    this.entry = entry;
  }
  ...
  withPlugins(pluginNames) {
    ...
    this.plugins = pluginNames.reduce((acc, name) => {
      const pluginApplied = this.entryList ? this.getEntryByName(this.entryList, `🦉${name}::apply`) || emptyEntry : emptyEntry;
      const pluginViewUpdated = this.entryList ? this.getEntryByName(this.entryList, `🦉${name}::view::update`) || emptyEntry : emptyEntry;
      const pluginOnEditorViewStateUpdated = this.entryList ? this.getEntryByName(this.entryList, `🦉${name}::onEditorViewStateUpdated`) || emptyEntry : emptyEntry;
      acc[name] = {
    ...
    if (this.stateApplied && pluginNames.length > 0) {
      ...
      const stateApplyOutlier = outlier(pluginEntries.map(([, {
        stateApply
      }]) => stateApply), this.options.outlierFactor);
      const viewUpdateOutlier = outlier(pluginEntries.map(([, {
        viewUpdate
      }]) => viewUpdate), this.options.outlierFactor);
       ...
       */
      const pluginIsOutlier = ([, methods]) => this.getMethodSum(methods) > budget && this.hasOutlierMethods(methods, {
        stateApplyOutlier,
        viewUpdateOutlier
      });
      this.slowPlugins = pluginEntries.filter(pluginIsOutlier).reduce((acc, [n, d]) => ({
        ...acc,
        [n]: d
      }), {});
    }
  ...
  withNodes(nodesCount, nodesDuration = 0) {
