...
const transformBrowserElementItem = item => {
  return {
    index: item.tempKey,
    title: item.title,
...
const parseCategories = itemCategories => {
  if (!itemCategories) {
    return ['apps'];
  }
...
const suggestedIds = ['table', 'action', 'codeblock', 'infopanel', 'threecolumnslayout', 'emoji', 'rule', 'expand'];

// slices items from the QuickInsertPanelItem[] into suggested, categories and search result items
export const useItems = (quickInsertPanelItems, query) => {
  const [searchText, setSearchText] = useState(query);
  const [selectedCategory, setSelectedCategory] = useState();
  const insertPanelItems = useMemo(() => getMappedItems(quickInsertPanelItems), [quickInsertPanelItems]);
