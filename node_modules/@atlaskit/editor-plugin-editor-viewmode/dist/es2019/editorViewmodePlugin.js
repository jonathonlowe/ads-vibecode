...
const viewModePluginKey = new PluginKey('editorViewMode');
const createPlugin = ({
  initialMode
}) => {
  return new SafePlugin({
    ...
    props: {
      ...
      editable: state => {
        ...
        const mode = (_viewModePluginKey$ge = viewModePluginKey.getState(state)) === null || _viewModePluginKey$ge === void 0 ? void 0 : _viewModePluginKey$ge.mode;
  ...
  switch (pluginConfig.initialContentMode) {
    ...
    case 'live-edit':
      ...
      return {
        ...
        mode: isEmptyDoc ? 'edit' : editorExperiment('live_pages_graceful_edit', 'control') ? 'edit' : editorExperiment('live_pages_graceful_edit', 'initially-hide-toolbar') ? 'edit' : 'view',
        isConsumption: isEmptyDoc ? false : editorExperiment('live_pages_graceful_edit', 'initially-hide-toolbar') ? false : true,
...
const gracefulEditCreatePMPlugin = ({
  ...
  const viewModeIntentToEdit = unstable_editorExperimentParam('live_pages_graceful_edit', 'view-mode-intent-to-edit', {
    defaultValue: false,
    typeGuard: value => typeof value === 'boolean'
  });
  return new SafePlugin({
    key: viewModePluginKey,
    state: {
      ...
      apply: (tr, pluginState) => {
        ...
        const action = tr.getMeta(viewModePluginKey);
        if (action) {
          switch (action.type) {
            case 'intent-to-edit':
              {
    ...
    props: {
      ...
      editable: state => {
        ...
        const mode = (_viewModePluginKey$ge2 = viewModePluginKey.getState(state)) === null || _viewModePluginKey$ge2 === void 0 ? void 0 : _viewModePluginKey$ge2.mode;
        return mode === 'view' ? false : undefined;
      },
      handleDOMEvents: {
        mousedown: (view, event) => {
          var _window$getSelection;
          const existingSelectionText = (_window$getSelection = window.getSelection()) === null || _window$getSelection === void 0 ? void 0 : _window$getSelection.toString();
      ...
      handleClick: (view, pos, event) => {
        ...
        const delayMs = !editorExperiment('live_pages_graceful_edit', 'text-click-no-delay') && unstable_editorExperimentParam('live_pages_graceful_edit', 'delay', {
          defaultValue: 200,
          typeGuard: value => typeof value === 'number'
        });
        if (delayMs) {
          if (timer) {
            clearTimeout(timer);
          ...
          timer = setTimeout(() => {
            var _api$core;
            api === null || api === void 0 ? void 0 : (_api$core = api.core) === null || _api$core === void 0 ? void 0 : _api$core.actions.execute(api === null || api === void 0 ? void 0 : api.editorViewMode.commands.updateContentMode({
              type: 'intent-to-edit'
            }));
            view.focus();
          }, delayMs);
        ...
        api === null || api === void 0 ? void 0 : (_api$core2 = api.core) === null || _api$core2 === void 0 ? void 0 : _api$core2.actions.execute(api === null || api === void 0 ? void 0 : api.editorViewMode.commands.updateContentMode({
          type: 'intent-to-edit'
        }));
        view.focus();
        return false;
...
function checkIntentToEdit(view, pos, event) {
  const viewModeIntentMode = unstable_editorExperimentParam('live_pages_graceful_edit', 'intent-mode', {
    defaultValue: 'text',
...
const gracefulEditTopToolbarCreatePMPlugin = config => {
  return new SafePlugin({
    key: viewModePluginKey,
    state: {
      init: (_, editorState) => {
    ...
    props: {
      ...
      editable: state => {
        ...
        const mode = (_viewModePluginKey$ge3 = viewModePluginKey.getState(state)) === null || _viewModePluginKey$ge3 === void 0 ? void 0 : _viewModePluginKey$ge3.mode;
        return mode === 'view' ? false : undefined;
      },
      handleClick: (view, pos, event) => {
        const viewModeState = viewModePluginKey.getState(view.state);
...
export const editorViewModeWithGracefulEditPlugin = ({
  config,
  api
}) => {
  if (!config) {
 ...
 */
export const editorViewModePlugin = ({
  config: options,
  api
}) => {
