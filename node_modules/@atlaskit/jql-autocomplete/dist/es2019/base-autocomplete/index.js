...
export class BaseAutocomplete {
   ...
   */
  constructor(parser, ignoredTokens = new Set(), preferredRules = new Set(), delimiterTokens = new Set()) {
    this.parser = parser;
   ...
   * @private
   */
  getMatchedText(maybeCaretToken, caretSelectionRange) {
    if (!maybeCaretToken) {
      return '';
    }
    ...
    const startTokenIndex = this.isCaretAtDelimiterOrHiddenToken(maybeCaretToken) ? maybeCaretToken.tokenIndex + 1 : maybeCaretToken.tokenIndex;
    return getMatchedText(this.getTokenStream(), startTokenIndex, maybeCaretToken, caretSelectionRange);
  }
   ...
   */
  getCandidateTokens(maybeCaretToken, caretSelectionRange, collectedCandidateTokens) {
    const candidateTokens = [];
    collectedCandidateTokens.forEach((value, key) => {
      const tokenSequence = [key, ...value]; // handle token sequences that are always used together, e.g. ORDER + BY
      const tokenName = tokenSequence.map(tokenType => this.parser.vocabulary.getDisplayName(tokenType).replace(/^'|'$/g, '')).join(' ');
      if (tokenName) {
    ...
    const replacePosition = this.getReplacePosition(maybeCaretToken, maybeCaretToken, caretSelectionRange);
    return {
      matchedText,
      replacePosition,
   ...
   */
  getCandidateRules(maybeCaretToken, caretSelectionRange, collectedCandidateRules) {
    const rules = this.mapCandidateRules(maybeCaretToken, caretSelectionRange, collectedCandidateRules);
   ...
   * will determine the `replacePosition` computed for the returned {@link Suggestions}.
   */
  mapCandidateRules(maybeCaretToken, caretSelectionRange, collectedCandidateRules) {
    const rules = new Map();
    const tokens = this.getTokenStream();
    collectedCandidateRules.forEach(({
      startTokenIndex,
      ruleList
    }, key) => {
      let defaultRule;
      const startToken = tokens.get(startTokenIndex);
      const replacePosition = this.getReplacePosition(startToken, maybeCaretToken, caretSelectionRange);
      if (!maybeCaretToken) {
        ...
        const matchedText = getMatchedText(tokens, startTokenIndex, maybeCaretToken, caretSelectionRange);
        defaultRule = {
          matchedText,
          replacePosition: replacePosition,
          context: null
   ...
   * @private
   */
  getCaretToken(caretStartPosition) {
   ...
   * @private
   */
  isCaretAtDelimiterToken(caretToken) {
   ...
   * @protected
   */
  isCaretAtDelimiterOrHiddenToken(caretToken) {
   ...
   * @protected
   */
  overrideReplacePosition(replaceStartToken, maybeCaretToken, caretSelectionRange) {
    return null;
  }
   ...
   * @private
   */
  getReplacePosition(replaceStartToken, maybeCaretToken, caretSelectionRange) {
    ...
    const overriddenReplacePosition = this.overrideReplacePosition(replaceStartToken, maybeCaretToken, caretSelectionRange);
    ...
    const startPosition = replaceStartToken === undefined || this.isCaretAtDelimiterOrHiddenToken(replaceStartToken) ? start : Math.min(replaceStartToken.startIndex, start);
    ...
    const stopPosition = maybeCaretToken === undefined || this.isCaretAtDelimiterOrHiddenToken(maybeCaretToken) ? stop : Math.max(maybeCaretToken.stopIndex + 1, stop);
   ...
   * will determine the `replacePosition` computed for the returned {@link Suggestions}.
   */
  getCandidates(maybeCaretToken, caretSelectionRange) {
    let caretTokenIndex;
    ...
    const tokens = this.getCandidateTokens(maybeCaretToken, caretSelectionRange, collectedCandidates.tokens);
    const rules = this.getCandidateRules(maybeCaretToken, caretSelectionRange, collectedCandidates.rules);
    return {
      tokens,
      rules
    };
   ...
   * will determine the `replacePosition` computed for the returned {@link Suggestions}.
   */
  getSuggestionsForCaretPosition(caretSelectionRange) {
   ...
   */
  getSuggestionsForTokenIndex(tokenIndex) {
    ...
    const caretSelectionRange = [caretToken.stopIndex + 1, caretToken.stopIndex + 1];
   ...
   * @protected
   */
  assignRuleContextData(maybeCaretToken, rules) {
    return rules;
  }
}