...
export class JQLAutocomplete extends BaseAutocomplete {
  /**
   ...
   */
  constructor(parser, ignoredTokens = defaultIgnoredTokens, preferredRules = defaultPreferredRules, delimiterTokens = defaultDelimiterTokens, tree) {
   ...
   */
  static fromText(text, ignoredTokens, preferredRules, delimiterTokens) {
   ...
   * @param delimiterTokens Tokens to use as delimiters. When not provided, {@link defaultDelimiterTokens} will be used.
   */
  static fromParser(parser, ignoredTokens, preferredRules, delimiterTokens) {
  ...
  assignRuleContextData(maybeCaretToken, rules) {
    ...
    rules.forEach(([ruleSuggestion, ruleList], ruleNumber) => {
      if (rulesWithContext.includes(ruleNumber)) {
        var _this$tree$accept, _this$tree;
        const ruleContextVisitor = new RuleContextVisitor(ruleSuggestion, ruleList, ruleNumber, this.getTokenStream(), maybeCaretToken);
        ruleSuggestion.context = (_this$tree$accept = (_this$tree = this.tree) === null || _this$tree === void 0 ? void 0 : _this$tree.accept(ruleContextVisitor)) !== null && _this$tree$accept !== void 0 ? _this$tree$accept : null;
      }
    });
    return rules;
  }
  getJQLSuggestionsForCaretPosition(caretSelectionRange) {
    return this.mapToJQLSuggestions(this.getSuggestionsForCaretPosition(caretSelectionRange));
  }
  getJQLSuggestionsForTokenIndex(tokenIndex) {
  ...
  isCaretAtUnclosedString(caretToken) {
