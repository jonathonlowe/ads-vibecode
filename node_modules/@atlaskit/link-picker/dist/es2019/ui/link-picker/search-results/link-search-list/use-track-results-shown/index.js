...
export const useTrackResultsShown = (isLoading, items, hasSearchTerm) => {
  ...
  const finalItems = useMemo(() => !isLoading ? items : null, [isLoading, items]);
  ...
  useEffect(() => {
    if (debouncedItems) {
      const event = !hasSearchTermRef.current ? 'ui.searchResults.shown.preQuerySearchResults' : 'ui.searchResults.shown.postQuerySearchResults';
