...
const isErrorWithStatus = error => error instanceof Error && `status` in error && typeof error.status === 'number';
export const processError = error => {
  if (isErrorWithStatus(error)) {
    const reason = createRequestErrorReason(error.status);
    return new MediaViewerError('docviewer-fetch-pdf', new RequestError(reason, {
      statusCode: error.status
    }));
  }
  return new MediaViewerError('docviewer-fetch-pdf',
