...
export class InteractiveImgComponent extends React.Component {
  constructor(...args) {
    ...
    _defineProperty(this, "saveWrapperRef", ref => this.wrapper = ref);
    ...
    _defineProperty(this, "onImgLoad", ev => {
      const {
        onLoad,
    ...
    _defineProperty(this, "onZoomChange", nextZoomLevel => {
      const {
        camera
      } = this.state;
      const {
    ...
    _defineProperty(this, "stopDragging", ev => {
      ev.preventDefault();
      this.setState({
        isDragging: false
      });
    });
    _defineProperty(this, "panImage", ev => {
  ...
  render() {
    ...
    const wrapperStyleOverride = camera ? {} : {
      overflow: 'hidden'
    };
    if (orientation) {
      imgStyle.transform = getCssFromImageOrientation(orientation);
    }
    const srcToDisplay = isHDAvailable && isHDActive && originalBinaryImageSrc ? originalBinaryImageSrc : src;
